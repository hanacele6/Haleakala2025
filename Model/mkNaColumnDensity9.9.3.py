...
日付: 2026/07/08 (Updated: Multi-Bin Model Integration)
==============================================================================
"""
...
# 値の意味:
#   'on'    : 常に有効 (既定)
#   'off'   : 最初から最後まで無効 (スピンアップから切る = 定常状態で比較できる)
#   'final' : 最終年(記録フェーズ)だけ無効 (定常なパターンに対する即時効果を見る)
#
# 例:
#   --srp off    : 放射圧を最初から切って新たな定常に到達させる
#   --srp final  : 最終年だけ放射圧を切る (系は定常でなくなる点に注意)
parser.add_argument('--srp', type=str, default='on', choices=['on', 'off', 'final'],
                    help='太陽輻射圧')
parser.add_argument('--td', type=str, default='on', choices=['on', 'off', 'final'],
                    help='熱脱離')
parser.add_argument('--psd', type=str, default='on', choices=['on', 'off', 'final'],
                    help='光刺激脱離')

# ==========================================================================
# 記録・保存の範囲
# ==========================================================================
# 'final'  : 最終年(記録フェーズ)のみ保存 (既定。容量が小さい)
# 'all'    : スピンアップ含め全期間を保存 (形成過程を追える。容量大)
parser.add_argument('--save_phase', type=str, default='final', choices=['final', 'all'],
                    help='スナップショットとホップ輸送を保存する範囲')

# スピンアップ年数と記録年数もバッチから変えられるようにする
parser.add_argument('--spinup_years', type=float, default=14.0)
parser.add_argument('--total_years', type=float, default=1.0)
...
# ★★★ ホップ輸送(放出位置→着地位置)の記録設定
# 放出された原子が「どこから出てどこに着地したか」を集計する。
# 座標は太陽相対の時角 h [deg]: h=0 が太陽直下点、h<0 が明け方側、h=-90 が明け方ターミネーター。
# 目的:
#   ・ターミネーター付近の高密度領域が、昼側からの輸送で作られているかの確認
#   ・PSD(1500K, 高速)とTD(表面温度, 低速)で飛距離が違うことの直接測定
#   ・経度方向の正味フラックス F(h) を求め、輸送が分布をどちらへ動かすかを見る
RECORD_HOP_TRANSPORT = True   # Falseにすると記録しない(わずかに高速化)
HOP_N_LON_BINS = 72           # 時角のビン数 (5度刻み)
HOP_N_TAA_BINS = 36           # TAAのビン数 (10度刻み)
HOP_N_PROC = 4                # 放出過程: 0=PSD, 1=TD, 2=SWS, 3=MMV
HOP_PROC_NAMES = ['PSD', 'TD', 'SWS', 'MMV']
...
SIMULATION_SETTINGS = {
    ...
    #'GRID_RESOLUTION': 301,  # 領域が倍になるため、分割数も約2倍にする
    #'GRID_MAX_RM': 15.0,     # 出力するヒストグラムの空間範囲を10 RMに拡張
    #'GRID_RADIUS_RM': 16.0,  # 粒子の計算打ち切り（エスケープ）境界を外側に押し出す
...
Target_Grain_Radius = 100.0e-6  # [m] これは書いてあるだけです
...
def calculate_single_g_factor_numba(px, py, pz, vx, V_rad, r0, spec_wl, spec_gamma, JL_const, AU):
    """
    生存している個々の粒子のその瞬間の速度から正確なg-factor [photons/s/atom] を計算する。
    """
    # 影に隠れている場合は発光しない (b=0)
...
@njit(fastmath=True)
def calculate_acceleration_numba(px, py, pz, vx, vy, vz, V_rad, V_tan, r0, spec_wl, spec_gamma, JL_const, AU, srp_on):
    r_sq = px**2 + py**2 + pz**2
    if r_sq > 0:
...
def update_particles_numba(pos, vel, status_mask, result_codes, particle_bins, dt_step, V_rad, V_tan, r0, R_MAX, tau_ion, 
                           ...
                           u_model_type, sticking_model, hop_tau_ratio, use_prob_sticking, temp_base, temp_amp, temp_night, u_max, u_min, srp_on):
    ...
    for i in prange(n_particles):
        ...
        while t_rem > 1e-6:
            ...
            if t_hit_est < dt:
                ...
                # --- 3. 軌道積分 (完全版 RK4) ---
                k1_vx, k1_vy, k1_vz = calculate_acceleration_numba(px, py, pz, vx, vy, vz, V_rad, V_tan, r0, spec_wl, spec_gamma, JL_const, AU, srp_on)
                ...
                k2_vx, k2_vy, k2_vz = calculate_acceleration_numba(px2, py2, pz2, vx2, vy2, vz2, V_rad, V_tan, r0, spec_wl, spec_gamma, JL_const, AU, srp_on)
                ...
                k3_vx, k3_vy, k3_vz = calculate_acceleration_numba(px3, py3, pz3, vx3, vy3, vz3, V_rad, V_tan, r0, spec_wl, spec_gamma, JL_const, AU, srp_on)
                ...
                k4_vx, k4_vy, k4_vz = calculate_acceleration_numba(px4, py4, pz4, vx4, vy4, vz4, V_rad, V_tan, r0, spec_wl, spec_gamma, JL_const, AU, srp_on)
...
def generate_particles_grid_numba(
    ...
    w_global, temp_psd, temp_night, temp_day_map, illum_frac_map,  # w_psdなどを削除し w_globalに統一
    ...
    particle_birth_h, particle_birth_proc, hop_n_lon_bins
):
    # band_gen_* : shape (n_bands,) 各eff_cosバンドでの生成量[atoms] (in-placeで加算)
    # band_sweep_*: shape (n_bands,) 【案1】各eff_cosバンドに"今存在する"表面在庫ストック[atoms]
    #               (遷移検出ではなく、昼側セルの現在の在庫を、そのセルの現在のバンドに毎回計上)
    #               ステップ積算値なので、解析側では Step_Count で割って時間平均ストックにする。
    ptr = 0
    ...
    # [0]PSD_Dawn [1]PSD_Dusk [2]TD_Dawn [3]TD_Dusk [4]SWS_Dawn [5]SWS_Dusk
    # [6]PSD_Dawn_Terminator(近傍のみ, [0]の内訳) [7]TD_Dawn_Terminator(近傍のみ, [2]の内訳)
    ...
    for i in range(n_lon):
        ...
        for j in range(n_lat):
            ...
            # nx は eff_cos そのもの。ターミネーター近傍(日照が始まったばかり)かどうかの判定に使う
            is_terminator_near = (nx > 0.0) and (nx <= eff_cos_terminator_max)

            # このセルが属する eff_cos バンドのインデックス(昼側のみ; 夜側は-1)
            band_k = -1
            if nx > 0.0:
            ...
            # --- 【案1】在庫ストックの記録: 昼側セルの現在の在庫を、現在のバンドに毎ステップ計上 ---
            # 「渋滞域(高密度の在庫の塊)が天頂角を横切って移動する間に、
            #   Qによってどう痩せるか」をTAA×バンドのストックとして可視化するため。
                ...
                for n_amount, temp_val, stat_dawn, stat_dusk in params:
                    if n_amount > 0.0:
                        ...
                        for _ in range(num):
                            ...
                            # --- ホップ輸送の記録: 放出時の時角ビンと放出過程 ---
                            # diff_rad は (セル経度 - 太陽直下点経度) を -pi..pi に畳んだ値 = 時角[rad]
                            hb = int((diff_rad + np.pi) / (2.0 * np.pi) * hop_n_lon_bins)
                            if hb < 0:
                                hb = 0
                            elif hb >= hop_n_lon_bins:
                                hb = hop_n_lon_bins - 1
                            particle_birth_h[idx] = hb
                            # stat_dawn: 0=PSD, 2=TD, 4=SWS  →  過程インデックス 0,1,2
                            particle_birth_proc[idx] = stat_dawn // 2

                            if is_dawn:
                                stats[stat_dawn] += w_global
                                # ターミネーター近傍の内訳を、PSD(stat_dawn==0)とTD(stat_dawn==2)のみ記録
...
def process_dead_particles_numba(pos, weight, result_codes, particle_bins,
                                 ...
                                 n_lon, n_lat, n_bins,
                                 particle_birth_h, particle_birth_proc,
                                 hop_hist, hop_taa_bin, hop_n_lon_bins, record_hop):
    """
    死んだ粒子を処理する統合関数。
    ...
    for i in range(pos.shape[0]):
        ...
        if code == 1:
            ...
            if r > 0.0:
                ...
                # --- ホップ輸送の記録: 放出時角ビン → 着地時角ビン ---
                # ln は太陽相対の経度(時角)。ln_fix に変換する前の値をそのまま使う。
                if record_hop:
                    hb_birth = particle_birth_h[i]
                    pr = particle_birth_proc[i]
                    hb_land = int((ln + np.pi) / (2.0 * np.pi) * hop_n_lon_bins)
                    if hb_land < 0:
                        hb_land = 0
                    elif hb_land >= hop_n_lon_bins:
                        hb_land = hop_n_lon_bins - 1
                    if (0 <= hb_birth < hop_n_lon_bins) and (0 <= pr < hop_hist.shape[1]):
                        hop_hist[hop_taa_bin, pr, hb_birth, hb_land] += w
    ...
    for i in prange(n_lon):
        ...
        for j in range(n_lat):
            ...
            for b in range(n_bins):
                ...
                r_psd = f_uv * q_psd_bins[b] * eff_cos * illum if psd_on else 0.0
                ...
                # 落下してきた分(gain_dens)を先に加味したうえで、空き容量を計算する
                room_left = max(0.0, capacity_b - (current_dens + gain_dens))
                
                # 地下からの拡散供給は、空き容量の範囲内でのみ許可する
...
def main_snapshot_simulation():
    ...
    # ★ ここでシードを完全に固定 ★
    # =================================================
    MASTER_SEED = 202606  # 好きな数字でOKです
    np.random.seed(MASTER_SEED)      # Python/NumPy側の乱数を固定 (MMV生成などに影響)
    set_numba_seed(MASTER_SEED)      # Numba内部の乱数を固定 (PSD, TD, update_particles等に影響)
    # =================================================
    ...
    SAVE_SPINUP_PHASE = (args.save_phase == 'all')
    ...
    SPIN_UP_YEARS = args.spinup_years
    TOTAL_SIM_YEARS = args.total_years
    ...
    # 1. 日付の取得 (MMDD形式、例: 0702)
    date_str = datetime.datetime.now().strftime("%m%d")
    
    # 2. 拡散モデル (BD) の判定
    # 'USE_STD_DIFFUSION' が True の場合のみ、活性化エネルギーを反映
    if SIMULATION_SETTINGS.get('USE_STD_DIFFUSION', False):
        bd_str = f"BD{args.diff_ea:.1f}"
    else:
        bd_str = "NonBD"
        
    # 3. 束縛エネルギーモデル (UG / UF) と中心値の判定
    ...
    # 7. 数値実験の条件をファイル名に反映
    #    on なら何も付けない。off / final のときだけタグを付ける。
    exp_tags = []
    for nm, val in (('SRP', args.srp), ('TD', args.td), ('PSD', args.psd)):
        if val == 'off':
            exp_tags.append(f"no{nm}")        # 最初から無効
        elif val == 'final':
            exp_tags.append(f"no{nm}Fin")     # 最終年だけ無効
    exp_str = ("_" + "_".join(exp_tags)) if exp_tags else ""

    # 8. 保存範囲
    save_str = "_SaveAll" if args.save_phase == 'all' else ""

    yr_str = f"{int(SPIN_UP_YEARS + TOTAL_SIM_YEARS)}yr"

    run_name = (f"New_ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_{date_str}"
                f"_{bd_str}_{u_str}_{q_str}_{a_str}_{lt_str}_{yr_str}{exp_str}{save_str}")
    # ==========================================================================
    ...
    print(f"Experiment: SRP={args.srp}, TD={args.td}, PSD={args.psd}, save_phase={args.save_phase}")
    print(f"Years: spinup={SPIN_UP_YEARS}, total={TOTAL_SIM_YEARS}")
    ...
    # ★ホップ輸送の記録用: 放出時の時角ビンと放出過程(0=PSD,1=TD,2=SWS,3=MMV)
    particle_birth_h = np.zeros(MAX_PARTICLES, dtype=np.int32)
    particle_birth_proc = np.zeros(MAX_PARTICLES, dtype=np.int32)
    ...
    # 掃き込み検出用: 前ステップ(前回generate呼び出し時)の照度。初期値-1は「未初期化」
    prev_illum_map = np.full((N_LON_FIXED, N_LAT), -1.0, dtype=np.float64)

    # バンド別統計: shape (360, N_EFF_COS_BANDS, 6)
    #   軸2: [0]Gen_PSD_Dawn [1]Gen_TD_Dawn [2]Gen_PSD_Dusk [3]Gen_TD_Dusk
    #        [4]Sweep_Dawn [5]Sweep_Dusk
    band_stats_data = np.zeros((360, N_EFF_COS_BANDS, 6), dtype=np.float64)

    # ★ホップ輸送ヒストグラム
    #   shape: (TAAビン, 放出過程, 放出時角ビン, 着地時角ビン)
    #   時角 h[deg] = -180 + (bin + 0.5) * 360/HOP_N_LON_BINS
    #     h=0    : 太陽直下点
    #     h=-90  : 明け方ターミネーター
    #     h=+90  : 夕方ターミネーター
    hop_hist = np.zeros((HOP_N_TAA_BINS, HOP_N_PROC,

                         HOP_N_LON_BINS, HOP_N_LON_BINS), dtype=np.float64)

    # ★ホップ輸送の年カウンタ
    #   経過時間ではなく近日点通過の回数で数えるので、
    #   軌道ファイルの開始TAAが0でなくても年番号がずれない。
    hop_year_counter = 0
    ...
    ts_accumulated_stats = {
        'Gen_Total': 0.0, 'Gen_PSD': 0.0, 'Gen_TD': 0.0, 'Gen_SWS': 0.0, 'Gen_MMV': 0.0,
        'Loss_Total': 0.0, 'Loss_Stuck': 0.0, 'Loss_Ionized': 0.0, 'Loss_Escaped': 0.0,
        'Supply_Internal': 0.0, 'Trans_Inward': 0.0
    }
    ...
    try:  # 🌟 Ctrl+Cをキャッチするための開始地点
        ...
        #with Pool(cpu_count() - 1) as pool:
            ...
            while t_curr < t_end_run:  # 🌟 ここから時間ステップのループ
                ...
                srp_on = not (args.srp == 'off' or (args.srp == 'final' and is_recording_phase))
                td_on  = not (args.td  == 'off' or (args.td  == 'final' and is_recording_phase))
                psd_on = not (args.psd == 'off' or (args.psd == 'final' and is_recording_phase))
                
                
                step_stats = {k: 0.0 for k in stats_data[0].keys()}
                illum_was_updated = False  # このステップで照度マップが更新されたか(掃き込み検出用)
                ...
                if time_since_last_update >= DT_RATE_UPDATE:
                    ...
                    # 掃き込み検出用: 照度マップが更新される「前」の値を退避しておく。
                    # 生成関数は (prev=旧照度, illum=新照度) を比較し、夜→昼の遷移を検出する。
                ...
                if n_mmv_total > 0:
                    ...
                    for _ in range(num_p):
                        ...
                        # ★ホップ輸送の記録: MMV は過程インデックス 3
                        _hb = int((diff_rad_mmv + np.pi) / (2.0 * np.pi) * HOP_N_LON_BINS)
                        particle_birth_h[idx] = min(max(_hb, 0), HOP_N_LON_BINS - 1)
                        particle_birth_proc[idx] = 3

                        step_stats['Gen_MMV'] += W_GLOBAL
                        if is_dawn_mmv: step_stats['Gen_MMV_Dawn'] += W_GLOBAL
                        else:           step_stats['Gen_MMV_Dusk'] += W_GLOBAL

                # --- グリッドからの放出 (PSD, TD, SWS をNumba処理) ---
                total_loss_step = cached_loss_rate_grid * DT_MOVE
                with np.errstate(divide='ignore', invalid='ignore'):
                    rate_tot = cached_rate_psd + cached_rate_td + cached_rate_sws
                ...
                # MMVで使った分を除いた残りの空き箱を渡す
                if empty_ptr < n_empty:
                    # バンド別集計用の一時配列(このステップ分)
                ...
                update_particles_numba(
                    ...
                    SIMULATION_SETTINGS['TEMP_BASE'], SIMULATION_SETTINGS['TEMP_AMP'], SIMULATION_SETTINGS['TEMP_NIGHT'],
                    SIMULATION_SETTINGS['U_MAX'], SIMULATION_SETTINGS['U_MIN'], srp_on,
                )

                # 死亡粒子の処理と、消滅理由の集計
                hop_taa_bin = int(TAA / 360.0 * HOP_N_TAA_BINS) % HOP_N_TAA_BINS
                loss_stats = process_dead_particles_numba(
                    ...
                    particle_birth_h, particle_birth_proc,
                    hop_hist, hop_taa_bin, HOP_N_LON_BINS,
                    RECORD_HOP_TRANSPORT and (is_recording_phase or SAVE_SPINUP_PHASE)
                )
                ...
                if is_recording_phase:
                    ...
                    for key in ts_accumulated_stats:
                        if key in step_stats:
                            ts_accumulated_stats[key] += step_stats[key]
                        elif key == 'Gen_Total':
                            ts_accumulated_stats['Gen_Total'] += (step_stats['Gen_PSD'] + step_stats['Gen_TD'] + step_stats['Gen_SWS'] + step_stats['Gen_MMV'])
                        elif key == 'Loss_Total':
                            ts_accumulated_stats['Loss_Total'] += (step_stats['Loss_Stuck'] + step_stats['Loss_Ionized'] + step_stats['Loss_Escaped'])
                    ...
                    def get_min_nonzero(arr):
                        valid_vals = arr[arr > 1e-20] # 完全にゼロのセルを排除
                ...
                if prev_taa != -999:
                    ...
                    if passed and (SAVE_SPINUP_PHASE or is_recording_phase):
                        ...
                        total_exosphere_atoms = np.sum(weight_array[status_mask])
                        total_surface_atoms = np.sum(surface_density * cell_areas[np.newaxis, :, np.newaxis])

                        timeseries_log.append({
                            'Time_hours': rel_h,
                            'TAA': TAA,
                            # ▼ 3-2. 追加・変更：現時点のストックと、累積した区間合計を記録
                            'Exosphere_Total': total_exosphere_atoms,
                            'Surface_Total': total_surface_atoms,
                            'Gen_Total': ts_accumulated_stats['Gen_Total'],
                            'Gen_PSD': ts_accumulated_stats['Gen_PSD'],
                            'Gen_TD': ts_accumulated_stats['Gen_TD'],
                            'Gen_SWS': ts_accumulated_stats['Gen_SWS'],
                            'Gen_MMV': ts_accumulated_stats['Gen_MMV'],
                            'Loss_Total': ts_accumulated_stats['Loss_Total'],
                            'Loss_Stuck': ts_accumulated_stats['Loss_Stuck'],
                            'Loss_Ionized': ts_accumulated_stats['Loss_Ionized'],
                            'Loss_Escaped': ts_accumulated_stats['Loss_Escaped'],
                            'Supply_Internal': ts_accumulated_stats['Supply_Internal'],
                            'Trans_Inward': ts_accumulated_stats['Trans_Inward']
                        })

                        for key in ts_accumulated_stats:
                            ts_accumulated_stats[key] = 0.0
                        ...
                        # 1. 数密度グリッドの保存 [atoms / m^3]
                        dgrid = H_dens / cvol
                        fname_d = f"density_grid_t{int(rel_h):05d}_taa{int(round(TAA)):03d}.npy"
                        np.save(os.path.join(target_output_dir, fname_d), dgrid)
                        
                        # 2. 発光強度(体積放射率)グリッドの保存 [photons / s / m^3]
                        if calc_emis_flag:
                            egrid = H_emis / cvol
                ...
                if prev_taa > 350 and TAA < 10:  # 年越し判定 (近日点を通過)
                    if RECORD_HOP_TRANSPORT and (is_recording_phase or SAVE_SPINUP_PHASE):
                        # 蓄積があるときだけ保存する。記録開始前の空の年越しでファイルを作らない。
                        if hop_hist.sum() > 0:
                            hop_year_counter += 1
                            hop_path = os.path.join(
                                target_output_dir,
                                f"hop_transport_hist_yr{hop_year_counter:02d}.npy")
                            np.save(hop_path, hop_hist)
                            print(f"[SAVE] 年越し: 第{hop_year_counter}期のホップ輸送データを保存 "
                                  f"(経過 {(t_curr - t_start_spinup)/MERCURY_YEAR:.2f} 年)")
                            hop_hist.fill(0.0)
    ...
    # === バンド別統計(eff_cos多段バンド)の書き出し ===
    # long形式: 各行が (TAA, band_index, band_lo, band_hi, 各量) を持つ
        ...
        for deg in range(360):
            ...
            for k in range(N_EFF_COS_BANDS):
                bwriter.writerow([
                    ...
                    f"{band_stats_data[deg, k, 0]:.4e}",  # Gen_PSD_Dawn
                    f"{band_stats_data[deg, k, 1]:.4e}",  # Gen_TD_Dawn
                    f"{band_stats_data[deg, k, 2]:.4e}",  # Gen_PSD_Dusk
                    f"{band_stats_data[deg, k, 3]:.4e}",  # Gen_TD_Dusk
                    f"{band_stats_data[deg, k, 4]:.4e}",  # Stock_Dawn (時間積算; Step_Countで割ると平均)
                    f"{band_stats_data[deg, k, 5]:.4e}",  # Stock_Dusk
                    sc,
                ])
    print(f"Band statistics saved to {band_csv_filename}")

    # === ホップ輸送ヒストグラムの書き出し ===
    if RECORD_HOP_TRANSPORT:
        # 最後の年越し以降の分 (通常は最終年)。年越し保存と番号が連続するようにする。
        if hop_hist.sum() > 0:
            hop_year_counter += 1
            hop_path = os.path.join(
                target_output_dir, f"hop_transport_hist_yr{hop_year_counter:02d}.npy")
        else:
            hop_path = os.path.join(target_output_dir, "hop_transport_hist.npy")
        np.save(hop_path, hop_hist)
        meta_path = os.path.join(target_output_dir, "hop_transport_meta.txt")
        with open(meta_path, 'w') as mf:
            mf.write("# hop_transport_hist.npy の説明\n")
            mf.write("# shape = (TAAビン, 放出過程, 放出時角ビン, 着地時角ビン)\n")
            mf.write(f"# HOP_N_TAA_BINS = {HOP_N_TAA_BINS}   (TAA刻み {360.0/HOP_N_TAA_BINS:.1f} deg)\n")
            mf.write(f"# HOP_N_PROC     = {HOP_N_PROC}   順に {HOP_PROC_NAMES}\n")
            mf.write(f"# HOP_N_LON_BINS = {HOP_N_LON_BINS}   (時角刻み {360.0/HOP_N_LON_BINS:.1f} deg)\n")
            mf.write("# 時角 h[deg] = -180 + (bin + 0.5) * 360/HOP_N_LON_BINS\n")
            mf.write("#   h = 0   : 太陽直下点\n")
            mf.write("#   h = -90 : 明け方ターミネーター\n")
            mf.write("#   h = +90 : 夕方ターミネーター\n")
            mf.write("# 値の単位: atoms (重み付き)\n")
            mf.write("# 記録対象: 地表に吸着(stuck)した粒子のみ。電離・脱出したものは含まない。\n")
            mf.write("# 放出から吸着までに複数回バウンドした場合、正味の移動として記録される。\n")

            mf.write("#\n")
            mf.write(f"# 年ごとのファイル: hop_transport_hist_yrNN.npy (NN は近日点通過ごとの通し番号, 1始まり)\n")
            mf.write(f"#   全 {hop_year_counter} 期分\n")
            mf.write(f"# SPIN_UP_YEARS = {SPIN_UP_YEARS}, TOTAL_SIM_YEARS = {TOTAL_SIM_YEARS}\n")
            mf.write(f"# save_phase = {args.save_phase}\n")
            mf.write(f"# HOP_TAU_RATIO = {SIMULATION_SETTINGS['HOP_TAU_RATIO']}\n")
            mf.write(f"#   0.0 なら即時マルチバウンドなし = 1ホップの変位\n")
            mf.write(f"# 数値実験: srp={args.srp}, td={args.td}, psd={args.psd}\n")
        print(f"Hop transport histogram saved to {hop_path}")
        tot = hop_hist.sum()
        if tot > 0:
            for pi_ in range(HOP_N_PROC):
                sub = hop_hist[:, pi_, :, :]
                if sub.sum() > 0:
                    bi = np.arange(HOP_N_LON_BINS)
                    h_c = -180.0 + (bi + 0.5) * (360.0 / HOP_N_LON_BINS)
                    m2 = sub.sum(axis=0)
                    disp = (h_c[None, :] - h_c[:, None])
                    disp = (disp + 180.0) % 360.0 - 180.0
                    mean_disp = float(np.sum(m2 * disp) / np.sum(m2))
                    print(f"  {HOP_PROC_NAMES[pi_]:>4}: 着地量 {sub.sum():.3e} atoms, "
                          f"平均正味変位 {mean_disp:+.2f} deg")
