...
def simulate_single_particle_for_density(args):
    ...
    if EJECTION_ANGLE_DISTRIBUTION == 'cosine':
        ...
        # 太陽に照らされた半球からランダム（等方的）に放出する。
        # 角度を -π/2 から +π/2 の範囲で一様にサンプリングする。
        source_angle_rad = np.random.uniform(-PI / 2.0, PI / 2.0)

    else:
        # デフォルトまたは不正な設定の場合はcosine分布を使用する
        print(f"警告: 不正な放出角度分布 '{EJECTION_ANGLE_DISTRIBUTION}' です。cosine分布を使用します。")
        p = np.random.random()
        source_angle_rad = np.arcsin(np.sqrt(p))
        if np.random.random() < 0.5:
            source_angle_rad *= -1.0
...
def main():
    ...
    # --- ファイル名と保存先フォルダの設定 ---
    base_name_template = f"density_map_beta{settings['BETA']:.2f}_Q1.0_MW_pl24_RA"
