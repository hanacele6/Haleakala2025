...
SIMULATION_SETTINGS = {
    ...
    'HOP_TAU_RATIO': 0.3,
    'USE_TAU_HOP': True, #残り時間からバウンドをさせるかさせないか
    ...
    #3Dグリッドデータの出力形式 ('density', 'count', または 'both')
    'GRID_OUTPUT_TYPE': 'density',
...
def update_particles_numba(pos, vel, status_mask, result_codes, particle_bins, 
                           hop_count_array, weight_array,
                           dt_step, V_rad, V_tan, r0, R_MAX, tau_ion, 
    ...
    for i in prange(n_particles):
        ...
        while t_rem > 1e-6:
            ...
            if is_hit:
                ...
                if use_tau_hop and tau_td <= HOP_TAU_THRESHOLD and t_rem > tau_td:
                    t_rem -= tau_td
                    hop_count_array[i] += weight_array[i]
                    rb_spd = sample_speed_from_flux_numba(temp_impact)
                    lx, ly, lz = sample_lambertian_direction_numba()
                    v_rb_x, v_rb_y, v_rb_z = transform_local_to_world_numba(lx, ly, lz, px_hit, py_hit, pz_hit)
                    px, py, pz = px_hit + (px_hit / RM_NB), py_hit + (py_hit / RM_NB), pz_hit + (pz_hit / RM_NB)
                ...
                for n_amount, temp_val, stat_dawn, stat_dusk in params:
                    if n_amount > 0.0:
                        ...
                        for _ in range(num):
                            ...
                            if is_dawn: stats[stat_dawn] += w_global
                            else:       stats[stat_dusk] += w_global

    return stats

@njit(fastmath=True)
def fast_3d_histogram_numba(pos, weights, status_mask, bins, gmin, gmax):
    grid = np.zeros((bins, bins, bins), dtype=np.float64)
...
def main_snapshot_simulation():
    ...
    run_name = f"ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_0624_Multi_BD0.5_UG_Q2.0_Bouncetau30s_A2.0_LongLT(Fulle)_5res_season3"
    ...
    hop_count_array = np.zeros(MAX_PARTICLES, dtype=np.float64)
        ...
        #with Pool(cpu_count() - 1) as pool:
            ...
            while t_curr < t_end_run:  # 🌟 ここから時間ステップのループ
                ...
                update_particles_numba(
                    ...
                    use_tau_hop=np.bool_(SIMULATION_SETTINGS['USE_TAU_HOP']), 
                )
                ...
                if is_recording_phase:
                    ...
                    tgt['Step_Count'] += 1
                    tgt['Hop_Count'] += np.sum(hop_count_array)
                hop_count_array[:] = 0
                ...
                if prev_taa != -999:
                    ...
                    if passed and (SAVE_SPINUP_PHASE or is_recording_phase):
                        ...
                        # H には Weight の総和（= そのセルの実際のナトリウム原子数）が入っている
                        H = fast_3d_histogram_numba(pos_array, weight_array, status_mask, GRID_RESOLUTION, gmin, gmax)

                        output_type = SIMULATION_SETTINGS.get('GRID_OUTPUT_TYPE', 'both')

                        # 密度グリッドの保存
                        if output_type in ['density', 'both']:
                            dgrid = H / cvol
                        ...
                        if output_type in ['count', 'both']:
                            fname_n = f"atom_count_grid_t{int(rel_h):05d}_taa{int(round(TAA)):03d}.npy"
                            np.save(os.path.join(target_output_dir, fname_n), H)
    ...
    with open(csv_filename, 'w', newline='') as f:
        ...
        for deg in range(360):
            ...
            total_particles = d['Loss_Stuck'] + d['Loss_Ionized'] + d['Loss_Escaped']
            hop_per_particle = d['Hop_Count'] / total_particles if total_particles > 0 else 0.0
            ...
            row = [
                ...
                f"{d['Hop_Count']:.4e}", f"{hop_per_particle:.4f}"
            ]
            writer.writerow(row)
