...
def get_total_acceleration(pos, vel):
    ...
    if USE_GRAVITY:
        r_sq = np.sum(pos ** 2)
        if r_sq > 0:
            grav_accel = -PHYSICAL_CONSTANTS['G'] * PHYSICAL_CONSTANTS['MASS_MERCURY'] * pos / (r_sq ** 1.5)
            accel += grav_accel
...
fig.update_layout(
    title=f'(TAA={TAA:.1f}°, Gravity:{USE_GRAVITY}, Rad.Pressure:{USE_RADIATION_PRESSURE})',
    scene=dict(
        xaxis_title='X [R_M]',
        yaxis_title='Y [R_M]',
        zaxis_title='Z [R_M]',
