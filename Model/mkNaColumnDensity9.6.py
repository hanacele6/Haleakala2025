...
SIMULATION_SETTINGS = {
    ...
    'USE_RANDOM_INITIAL_TIME': True, # ランダム法（初期時間オフセット）のオンオフ
    'HOP_TAU_RATIO': 0.3,            # HOP_TAU_THRESHOLD を DT_MOVE の何割にするか (例: 0.3 なら 30%)
...
def update_particles_numba(pos, vel, status_mask, result_codes, particle_bins, dt_rem_array, dt_step, V_rad, V_tan, r0, R_MAX, tau_ion, 
                           spec_wl, spec_gamma, JL_const, AU, subsolar_lon, U_bins, V_weights, hop_tau_ratio,
    ...
    for i in prange(n_particles):
        ...
        if status_mask[i]:
            if t_rem < 0.0:
                dt_rem_array[i] = dt_step + t_rem 
            else:
                dt_rem_array[i] = dt_step
...
def generate_particles_grid_numba(
    ...
    total_loss_step, frac_psd, frac_td, frac_sws, dt_rem_array, dt_step, use_random_time,
                ...
                for n_amount, temp_val, stat_dawn, stat_dusk in params:
                    if n_amount > 0.0:
                        ...
                        for _ in range(num):
                            ...
                            if use_random_time:
                                dt_rem_array[idx] = dt_step * np.random.random()
                            else:
                                dt_rem_array[idx] = dt_step
...
def main_snapshot_simulation():
    ...
    run_name = f"ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_0610_Multi_BD0.5_UG_Q2.0_Bouncetau30s_A2.0_LongLT(Fulle)_RMtest4"
    ...
    dt_rem_array = np.full(MAX_PARTICLES, DT_MOVE, dtype=np.float64)
            ...
            while t_curr < t_end_run:  # 🌟 ここから時間ステップのループ
                ...
                if n_mmv_total > 0:
                    ...
                    for _ in range(num_p):
                        ...
                        if SIMULATION_SETTINGS['USE_RANDOM_INITIAL_TIME']:
                            dt_rem_array[idx] = DT_MOVE * np.random.random()
                        else:
                            dt_rem_array[idx] = DT_MOVE
                ...
                if empty_ptr < n_empty:
                    gen_stats = generate_particles_grid_numba(
                        ...
                        dt_rem_array, DT_MOVE, SIMULATION_SETTINGS['USE_RANDOM_INITIAL_TIME'], 
                ...
                update_particles_numba(
                    pos_array, vel_array, status_mask, result_codes, particle_bins, dt_rem_array,
                    DT_MOVE, V_rad, V_tan, r0, r_max_val, tau_ion, 
                    spec_dict['wl'], spec_dict['gamma'], spec_dict['JL'], AU, sub_lon,
                    U_BINS_ARRAY, V_WEIGHTS_ARRAY,
                    SIMULATION_SETTINGS['HOP_TAU_RATIO'],
                    u_mode_flag, stick_model_flag,
                ...
                if step_count <= 5:
                    ...
                    # 2. 空間にいる粒子の数と重み
                    active_count = np.sum(status_mask) # Python版の場合は len(active_particles)
                    active_weight = np.sum(weight_array[status_mask]) # Python版の場合は sum(p['weight'] for p in active_particles)
