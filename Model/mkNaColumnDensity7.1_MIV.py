...
(微小隕石衝突気化 - Micro-meteoroid Impact Vaporization - 専用バージョン)
...
Leblanc (2003) に基づき、粒子生成源として
**微小隕石衝突気化 (MIV)** のみを考慮します。
...
主な物理モデル (MIV版)
==============================================================================
1.  **粒子生成 (Micro-meteoroid Impact Vaporization, MIV)**:
    -   水星の太陽距離(AU)に依存する微小隕石の総フラックスに基づき、
        原子が全球から生成されます [cite: 341, 342]。
    -   フラックスは公転の進行方向(明け側, +Y)で、反対側(暮れ側, -Y)の
        2倍になるよう経度依存性を持たせています [cite: 332, 333]。
    -   MIVは表面のNa濃度や温度に依存しません 。

2.  **初期速度**:
    -   放出される原子の速さ: 固定温度 (MIV_TEMPERATURE = 3000 K) における
        マクスウェル分布に従います 。
...
# --- 変更 (MIV) ---
# MIV (微小隕石衝突気化) モデル用の物理定数を追加 (Leblanc 2003, Sec 2.4)
# 近日点におけるNaの総放出率 [atoms/s]
MIV_TOTAL_FLUX_PERIHELION_S = 5.0e23
# 太陽距離依存のべき指数 (R_Hel ^ -1.9)
MIV_R_HEL_POWER_LAW = 1.9
# MIVによる放出粒子のマクスウェル分布温度 [K]
...
def calculate_surface_temperature(lon_rad, lat_rad, AU, subsolar_lon_rad):
    """
    水星表面の局所的な温度を計算します。 (※表面衝突時の計算にのみ使用)
    ...
    # 日照側の温度 T_sun = T1 * (cos(SZA)^0.25) * (R_AU^-0.5)
...
def main_snapshot_simulation():
    ...
    # --- 削除 (TD/PSD) ---
    # N_LON, N_LAT (グリッド生成は不要)
    # CONSTANT_SURFACE_DENSITY (MIVは表面密度に依存しない )
    # --- 削除 (TD/PSD) ここまで ---
    ...
    print(f"--- 物理モデル設定 (MIV Only) ---")
    print(f"Flux at Perihelion: {MIV_TOTAL_FLUX_PERIHELION_S:.1e} atoms/s")
    print(f"R_Hel Power Law: -{MIV_R_HEL_POWER_LAW}")
    print(f"Ejecta Temperature: {MIV_TEMPERATURE} K")
    ...
    print(f"近日点距離: {AU_PERIHELION:.4f} AU")
    ...
    with tqdm(total=len(time_steps), desc="Time Evolution") as pbar:
        ...
        for t_sec in time_steps:
            ...
            # --- 4b. 表面から新しい粒子を生成 (MIV: 微小隕石衝突気化) ---
            newly_launched_particles = []

            # 1. 現在の太陽距離(AU)に基づいて、総放出率をスケーリング
            # Flux(AU) = Flux_peri * (AU_peri / AU)^1.9
            current_total_flux_s = MIV_TOTAL_FLUX_PERIHELION_S * (
                    (AU_PERIHELION / AU) ** MIV_R_HEL_POWER_LAW)

            # 2. このタイムステップで生成すべき総原子数を計算
            n_atoms_to_desorb = current_total_flux_s * TIME_STEP_SEC
            if n_atoms_to_desorb <= 0:
                num_to_launch_int = 0
            else:
                # 3. 生成すべき超粒子(SP)の数を計算 (確率的)
            ...
            if num_to_launch_int > 0:
                ...
                # リジェクションサンプリング用の定数 M (P(phi)の最大値 / g(phi)の最大値)
                # P(phi) propto (1 + (1/3)sin(phi)), max P = (4/3)
                # g(phi) = 1/(2pi) (一様分布)
                # M = max(P/g) propto max( (1+(1/3)sin(phi)) / (1) ) = 4/3
                M_rejection = 4.0 / 3.0

                for _ in range(num_to_launch_int):

                    # --- 経度のサンプリング (リジェクション法) ---
                    # P(phi) propto (1 + (1/3)sin(phi)) で分布させる
                    while True:
                        # 1. 候補となる経度 phi を [-pi, pi] から一様に選ぶ
                    ...
                    # 速度: MIV用の固定温度 (3000 K) でのマクスウェル分布
