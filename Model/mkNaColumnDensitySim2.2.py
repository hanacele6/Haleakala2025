...
def main():
    ...
    # パラメータが変わった場合も、この一行を修正すればOK
    base_name_template = f"density_map_beta{settings['BETA']:.2f}_Q1.0_MW"

    # 1. 共通部分を使ってサブフォルダの名前を決定
    sub_folder_name = base_name_template

    # 2. サブフォルダのフルパスを作成し、一度だけ作成する
    ...
    for line in orbit_lines:
        ...
        Q_PSD_cm2 = 1.0e-20 / 1e4 #Killenら (2007)
        ...
        R_PSD_peak_per_cm2 = F_UV_current_per_cm2 * Q_PSD_cm2 * cNa # F_UV * Q_PSD * c_Na * 1
        ...
        # 柱密度をまず [atoms/m^2] で計算 (J_0 * S / N_tp) * (Δt / V)
