...
def sample_speed_from_flux_numba(temp_k):
    ...
    u1 = np.random.random()
    u2 = np.random.random()
    E_joule = -kT * (np.log(u1) + np.log(u2))  # gamma(2,kT)と等価
...
@njit()
def generate_mmv_numba(
    pos_array, vel_array, weight_array, status_mask, 
    active_count, max_particles, n_mmv_total, w_mmv, temp_mmv, sub_lon, RM
):
    stats = np.zeros(3, dtype=np.float64) # [Total, Dawn, Dusk]
    
    num_f = n_mmv_total / w_mmv
    num_p = int(num_f)
    if np.random.random() < (num_f - num_p): 
        num_p += 1
    ...
    for _ in range(num_p):
        ...
        latr = np.arcsin(np.random.uniform(-1.0, 1.0))
        diff_rad_mmv = (lr - sub_lon + np.pi) % (2.0 * np.pi) - np.pi
        is_dawn_mmv = (diff_rad_mmv < 0)

        nx = np.cos(latr) * np.cos(lr)
        ny = np.cos(latr) * np.sin(lr)
        nz = np.sin(latr)
        
        spd = sample_speed_from_flux_numba(temp_mmv)
        lx, ly, lz = sample_lambertian_direction_numba()
...
def generate_particles_grid_numba(
    pos_array, vel_array, weight_array, status_mask, active_count, max_particles,
    ...
    for i in range(n_lon):
        ...
        for j in range(n_lat):
            ...
            for b in range(n_bins):
                ...
                # ★ それぞれの重み(w_psd等)を一緒に持たせる
                params = [
                    (n_lost * frac_psd[i, j, b], temp_psd, w_psd, 0, 1),
                    (n_lost * frac_td[i, j, b], temp_eff_td, w_td, 2, 3),
                    (n_lost * frac_sws[i, j, b], -1.0, w_sws, 4, 5) 
                ]
                
                for n_amount, temp_val, w_type, stat_dawn, stat_dusk in params:
                    if n_amount > 0.0:
                        num_f = n_amount / w_type
                        num = int(num_f)
                        ...
                        for _ in range(num):
                            ...
                            pos_array[idx, 0], pos_array[idx, 1], pos_array[idx, 2] = nx * RM, ny * RM, nz * RM
                            vel_array[idx, 0], vel_array[idx, 1], vel_array[idx, 2] = vx * spd, vy * spd, vz * spd
                            weight_array[idx] = w_type
                            status_mask[idx] = True
                            
                            if is_dawn: stats[stat_dawn] += w_type
                            else:       stats[stat_dusk] += w_type

    return stats, active_count

@njit()
def compact_particles_numba(pos, vel, weight, status_mask, result_codes, particle_bins, active_count):
    """Listの削除操作を再現し、生きている粒子を順番を保ったまま前に詰める"""
    write_idx = 0
    for read_idx in range(active_count):
        if status_mask[read_idx]:
            if write_idx != read_idx:
                pos[write_idx] = pos[read_idx]
                vel[write_idx] = vel[read_idx]
                weight[write_idx] = weight[read_idx]
                status_mask[write_idx] = status_mask[read_idx]
                result_codes[write_idx] = result_codes[read_idx]
                particle_bins[write_idx] = particle_bins[read_idx]
            write_idx += 1
    # 残りの要素をクリーンアップ
    for i in range(write_idx, active_count):
        status_mask[i] = False
        result_codes[i] = 0
    return write_idx
...
def main_snapshot_simulation():
    ...
    run_name = f"ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_0622_Multi_BD0.5_UG_Q2.0_Bouncetau30s_A2.0_LongLT(Fulle)_Test_3"
    ...
    try:  # 🌟 Ctrl+Cをキャッチするための開始地点
        ...
        #with Pool(cpu_count() - 1) as pool:
            ...
            while t_curr < t_end_run:  # 🌟 ここから時間ステップのループ
                ...
                # ★ 本来の重み計算を復活
                w_mmv = max(1.0, n_mmv_total / (TARGET_SPS['MMV'] * (DT_MOVE / DT_RATE_UPDATE)))

                if n_mmv_total > 0:
                    mmv_stats, active_count = generate_mmv_numba(
                        pos_array, vel_array, weight_array, status_mask,
                        active_count, MAX_PARTICLES, n_mmv_total, w_mmv,
                        TEMP_MMV, sub_lon, PHYSICAL_CONSTANTS['RM']
                    )
                    step_stats['Gen_MMV'] += mmv_stats[0]
                    step_stats['Gen_MMV_Dawn'] += mmv_stats[1]
                    step_stats['Gen_MMV_Dusk'] += mmv_stats[2]

                # --- グリッドからの放出 (PSD, TD, SWS) ---
                ...
                gen_stats, active_count = generate_particles_grid_numba(
                    pos_array, vel_array, weight_array, status_mask, active_count, MAX_PARTICLES,
                    total_loss_step, frac_psd, frac_td, frac_sws,
                ...
                active_count = compact_particles_numba(
                pos_array, vel_array, weight_array, status_mask, result_codes, particle_bins, active_count
                )
                ...
                if step_count <= 5:
                    ...
                    # 現在の重み総和
                    active_weight = np.sum(weight_array[:active_count])
                    print(f"  [Active]  Total Weight: {active_weight:.4e}")
                    # 生成と消滅のバランス
                    total_gen = step_stats['Gen_PSD'] + step_stats['Gen_TD'] + step_stats['Gen_SWS'] + step_stats['Gen_MMV']
                    total_loss = step_stats['Loss_Stuck'] + step_stats['Loss_Ionized'] + step_stats['Loss_Escaped']
                    print(f"  [Flux]    Generated   : +{total_gen:.4e}")
                    print(f"  [Flux]    Lost        : -{total_loss:.4e}")
