...
import sys
import os
# from multiprocessing import Pool, cpu_count # <- 削除
...
def main_smyth_method():
    ...
    for line in orbit_lines:
        ...
        # ★★★★★ 変更点: マルチプロセッシングを単純なループに置き換え ★★★★★
        results = [
            simulate_for_migration_time(task)
            for task in tqdm(tasks, desc=f"Calculating t_M for TAA={TAA:.1f}", leave=False)
        ]
        # ★★★★★ 変更点ここまで ★★★★★
    ...
    with open(csv_filepath, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['TAA_deg', 't_M_hr', 'tau_hr', 'N_bar_cm-2'])
