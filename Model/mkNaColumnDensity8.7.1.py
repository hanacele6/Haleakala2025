...
# ==============================================================================
# 4. メインルーチン (タイムステップ分離版)
# ==============================================================================
def main_snapshot_simulation():
    start_time = time.time()

    # --- A. シミュレーション設定 ---
    OUTPUT_DIRECTORY = r"./SimulationResult_DualStep"

    # === 【変更点1】タイムステップの分離 ===
    # 粒子の移動・軌道更新を行う間隔 (精度用: 短く)
    DT_MOVE = 250.0

    # 表面温度・生成レート・平衡密度を更新する間隔 (高速化用: 長く)
    # ※ DT_MOVE の整数倍にすることを推奨
    ...
    # スーパーパーティクル(SP)の目標数 (DT_RATE_UPDATE 期間あたりの目標数に換算して使用)
    ...
    cached_loss_rate_grid = np.zeros(surface_density.shape)  # 1秒あたりの放出数(密度×レート)

    # 累積用変数 (Accumulator)
    accumulated_gained_grid = np.zeros_like(surface_density)  # 複数ステップ分の再付着数を溜める
    time_since_last_update = DT_RATE_UPDATE * 2.0  # 最初は強制的に更新させるため大きく設定
    ...
    with tqdm(total=total_steps, desc="Simulating") as pbar:
        while t_curr < t_end_run:
            ...
            if time_since_last_update >= DT_RATE_UPDATE:
                ...
                # --- B. 表面グリッド計算 (重い処理) ---
                # 各グリッドの「1秒あたりの放出確率(rate)」と「1秒あたりの放出数(loss)」を計算してキャッシュ

                # 平衡解用: この期間(dt_accumulated)の平均Flux In [atoms/m^2/s]
                avg_flux_in = (accumulated_gained_grid / cell_areas) / dt_accumulated

                # 密度更新 (Gardening項含む)
                supply_dens = mmv_flux * dt_accumulated
                # ※ 前回のキャッシュされたLossを使うのが厳密だが、ここでは近似的に処理
                # (Strictな保存則が必要な場合は、Lossもaccumulateする必要あり)
                ...
                for i in range(N_LON_FIXED):
                    for j in range(N_LAT):
                        ...
                        # 密度更新: 前回の期間の収支を反映
                        # New = Old + In - Out + Supply
                        # Out項は「前回のレート」に基づくべきだが、簡略化のためここでのレートを使用
                        # または、純粋に equilibrium mode に頼る設計にする
                        if not np.isnan(na_eq_record[i, j]):
                            surface_density[i, j] = na_eq_record[i, j]
                        else:
                            # 非平衡領域の更新
                            # Out = dens * rate * dt
                            n_out = dens * rate_total * dt_accumulated
                            # In = accumulated_gained[i,j]
                            surface_density[i, j] += accumulated_gained_grid[i, j] / cell_areas[j] - n_out + supply_dens

                        # 負にならないようClip
                        if surface_density[i, j] < 0: surface_density[i, j] = 0

                        # ★ キャッシュ更新: 「今の密度」での「1秒あたりの放出数」
                        n_avail = surface_density[i, j] * cell_areas[j]
                        # 1秒で放出される量 (rate * N)
                        temp_loss_per_sec[i, j] = n_avail * rate_total
            ...
            # 3. 粒子の生成 (毎 DT_MOVE ごとに、キャッシュに基づいて生成)
            new_particles = []

            # A. MMV (全球)
            mmv_flux = calculate_mmv_flux(AU)  # 距離は変わっているので再計算
            n_mmv = mmv_flux * 4 * np.pi * PHYSICAL_CONSTANTS['RM'] ** 2 * DT_MOVE
            w_mmv = max(1.0, n_mmv / (TARGET_SPS['MMV'] * (DT_MOVE / DT_RATE_UPDATE)))
            # ※ TARGET_SPSはレート更新期間あたりの数なので、DT_MOVE分にスケーリング
            ...
            # B. 表面プロセス (キャッシュ使用)
            # 全体の重み計算
            # 今のステップ(DT_MOVE)で放出される総粒子数予測
            total_loss_step = cached_loss_rate_grid * DT_MOVE

            # 各プロセスの寄与率マップ (分母が0の場合は0)
            ...
            # 生成ループ (ここでの n_lost はこの DT_MOVE 分の放出量)
            # ※ 計算高速化のため、Lossがゼロの場所はスキップしたいが、
            # 2次元配列操作はNumpyでまとめてやるか、既存通りループするか。
            # ここでは既存ロジックを維持しつつキャッシュ値を使用。

            # 放出がある場所だけ抽出してループすると高速 (np.argwhere等)
            # 簡略化のため元の構造を維持します
            for i in range(N_LON_FIXED):
                for j in range(N_LAT):
            ...
            if prev_taa != -999:
                ...
                if passed and t_curr >= t_start_run:
                    ...
                    print(f" Saving TAA={TAA:.1f}, Particles={len(active_particles)}")
