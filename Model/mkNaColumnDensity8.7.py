...
def main_snapshot_simulation():
    ...
    with tqdm(total=total_steps, desc="Simulating") as pbar:
        while t_curr < t_end_run:
            ...
            # 【追加】表面供給 (Surface Supply / Gardening)
            # 論文 (Leblanc & Johnson 2003) Discussion 5.1 & Conclusion に基づく実装。
            # 微小隕石(MMV)による「直接放出」とは別に、衝突による「表面の掘り起こし」によって
            # 新鮮なNaが供給される効果をモデル化します。
            #
            # 供給レートは MMV の直接放出フラックスと同程度と仮定されます。
            # mmv_flux: [atoms/m^2/s] (ループ冒頭で計算済み)
            # MAIN_DT:  [s]
            supply_dens = mmv_flux * MAIN_DT

            # 密度更新式: New = Old + 再付着 - 放出 + 内部からの供給
            dens_next = surface_density + gain_dens - loss_dens + supply_dens

            # 平衡モードのグリッドは計算値を上書き (供給項の影響は平衡計算ロジック内で吸収されるためそのまま)
