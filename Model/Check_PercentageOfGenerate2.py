...
def main():
    ...
    # ==========================================================================
    # グラフ1: 全球平均フラックスの対数推移 + Total Flux
    # ==========================================================================
    print("Generating Log-Flux Plot with Total...")
    plt.figure(figsize=(10, 6))

    # Total Flux を計算
    flux_total = flux_psd + flux_td + flux_sws + flux_mmv

    # --- Simulation Results ---

    # Total Flux (一番目立たせるため黒で太く)
    #plt.plot(result_taa, flux_total, label='Total Flux', color='black', linewidth=2.5, zorder=10)

    # 各成分
    plt.plot(result_taa, flux_psd, label='Sim: PSD', color='blue', linewidth=2)
    plt.plot(result_taa, flux_td, label='Sim: TD', color='red', linewidth=2)
    ...
    #plt.title(f"Source Flux Evolution (Total & Components)", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig("source_flux_logscale_total.png", dpi=300)
    print("Saved: source_flux_logscale_total.png")

    # ==========================================================================
    # グラフ2: 寄与率(%) + Total Flux (2軸表示)
    # ==========================================================================
    print("Generating Contribution Rate & Total Flux Plot...")

    # 以前の plt.figure() ではなく、fig, ax を作る形に変更
    fig, ax1 = plt.subplots(figsize=(10, 6))

    total_rate = rate_td + rate_psd + rate_sws + rate_mmv

    # 全球平均Fluxの合計値 (cm^-2 s^-1)
    total_flux_cm2 = flux_td + flux_psd + flux_sws + flux_mmv
    ...
    # --- 左軸 (ax1): 寄与率の積み上げ ---
    ax1.stackplot(result_taa, p_td, p_psd, p_sws, p_mmv,
                  labels=['TD', 'PSD', 'SWS', 'MMV'],
                  colors=['blue', 'red', 'green', 'orange'], alpha=0.6)  # 線が見えるように少し透明に

    ax1.set_xlim(0, 360)
    ax1.set_ylim(0, 100)
    ax1.set_xlabel("True Anomaly Angle (TAA) [deg]", fontsize=12)
    ax1.set_ylabel("Contribution Ratio [%]", fontsize=12)
    ax1.grid(True, alpha=0.3)

    # --- 右軸 (ax2): Total Flux ---
    ax2 = ax1.twinx()  # ax1とx軸を共有する新しい軸を作成

    # Total Flux をプロット (黒の太い破線などにする)
    #ax2.plot(result_taa, total_flux_cm2, color='black', linewidth=2.5, linestyle='--', label='Total Flux')

    ax2.set_ylabel("Total Flux [atoms cm$^{-2}$ s$^{-1}$]", fontsize=12, color='black')
    ax2.set_yscale('log')  # 変動が大きいので対数軸が見やすい
    ax2.tick_params(axis='y', labelcolor='black')

    # タイトル
    #plt.title("Source Contribution Ratio & Total Flux Trend", fontsize=14)

    # 凡例をまとめて表示 (左軸の面と、右軸の線を一緒に)
    lines_1, labels_1 = ax1.get_legend_handles_labels()
    lines_2, labels_2 = ax2.get_legend_handles_labels()
    # グラフの外側に配置
    ax1.legend(lines_1 + lines_2, labels_1 + labels_2, loc='upper right', bbox_to_anchor=(1.25, 1.0))

    plt.tight_layout()
    plt.savefig("source_contribution_with_total.png", dpi=300)
    print("Saved: source_contribution_with_total.png")
