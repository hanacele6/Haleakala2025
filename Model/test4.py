...
def load_band_stats(model_dir, side):
    ...
    # 【案1】在庫ストック列 (Stock_*)。旧形式(Sweep_*)も一応フォールバック。
    if f"Stock_{side}" in df.columns:
        stock_col = f"Stock_{side}"
    else:
        stock_col = f"Sweep_{side}"

    df['Gen_PSD'] = df[gen_psd_col]
    df['Gen_TD'] = df[gen_td_col]
    df['Gen_Total'] = df['Gen_PSD'] + df['Gen_TD']

    # ストックは時間積算なので Step_Count で割って「時間平均ストック」にする
...
def plot_heatmaps(data_dict):
    for label, d in data_dict.items():
        ...
        for ax, key, title in zip(
            axes,
            ['Stock', 'Gen_PSD', 'Gen_TD'],
            ['表面在庫ストック(時間平均)', 'PSD生成量', 'TD生成量']
        ):
...
def plot_dayside_totals(data_dict):
    ...
    for label, d in data_dict.items():
        taa = d['taa']
        c = colors[label]
        sweep_tot = circular_smooth_2d(d['grids']['Stock'], SMOOTH_WINDOW_DEG).sum(axis=1)
    ...
    titles = ['昼全体の在庫ストック(時間平均)', '昼全体のPSD生成量', '昼全体のTD生成量']
    for ax, t in zip(axes, titles):
        ax.set_ylabel('atoms')
        ax.set_title(f'{t} ({SIDE}側)')
        ax.axvline(180, color='gray', ls=':', alpha=0.6, label='遠日点')
        #ax.axvline(100, color='orange', ls='--', alpha=0.5, label='TAA=100')
...
def plot_emission_centroid(data_dict):
    ...
    for label, d in data_dict.items():
        ...
        sweep = circular_smooth_2d(d['grids']['Stock'], SMOOTH_WINDOW_DEG)
        ...
        axes[1].plot(taa, sweep_centroid, color=c, lw=2, ls='--', label=f'{label}: 在庫ストックの重心')

    axes[0].set_ylabel('放出の局所時刻重心\n(eff_cos加重平均)')
    axes[0].set_title(f'放出が起きた局所時刻の重心 ({SIDE}側)\n'
                      f'高い=正午寄り(TD側)、低い=ターミネーター寄り(PSD側)')
    axes[1].set_ylabel('在庫ストックの局所時刻重心\n(eff_cos加重平均)')
    axes[1].set_title('表面在庫ストックの局所時刻重心')
...
def print_summary(data_dict):
    ...
    for label, d in data_dict.items():
        taa = d['taa']
        sweep_tot = d['grids']['Stock'].sum(axis=1)
        ...
        print(f"\n[{label}]")
        print(f"  在庫ストックpiピーク TAA : {peak(sweep_tot):.0f} deg")
