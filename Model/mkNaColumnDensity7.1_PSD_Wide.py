...
水星の公転運動に伴う太陽との位置関係（距離、速度）の変化を反映させながら、
表面からのナトリウム原子の生成、宇宙空間での運動、そして消滅過程を追跡します。

シミュレーションは水星の公転周期に沿って進行し、指定された真近点離角 (TAA) に
達するたびに、全粒子の状態を3次元の立方体グリッド上の密度分布として
集計・保存します。
...
この座標系を採用することにより、太陽放射圧と太陽重力は常にX軸に沿った力として
（またはX軸を基準として）計算できますが、水星が公転していることによる
「見かけの力」（遠心力とコリオリ力）を考慮する必要があります。

==============================================================================
主な物理モデル
==============================================================================
1.  **粒子生成 (Photo-Stimulated Desorption, PSD)**:
    -   太陽からの紫外線フラックス、太陽天頂角(SZA)、表面のナトリウム密度に基づき、
        日照側の表面から粒子が生成されます。
    -   本バージョンでは、表面密度は無限供給源（常に一定）と仮定しています。

2.  **初期速度**:
    -   放出される原子の速さ: 指定された表面温度(SOURCE_TEMPERATURE)における
        マクスウェル分布に従います。
...
def calculate_surface_temperature(lon_rad, lat_rad, AU, subsolar_lon_rad):
    ...
    # (0.306 / AU)^2 = (0.553 / R_AU)^2 ... R_AU = AU / 0.306?
    # TODO: この式の出典を確認。一般的には (1/AU^2)^0.25 = 1/sqrt(AU)
    # ここでは既存のコードのロジックを維持
...
def get_orbital_params(time_sec, orbit_data, mercury_year_sec):
    ...
    # 太陽直下点経度は、シミュレーション開始からの総経過時間(time_sec)と
    # 水星の自転周期に基づいて計算される（公転とは独立して回転し続ける）
...
def main_snapshot_simulation():
    ...
    # 出力ディレクトリの準備
    run_name = f"Grid{GRID_RESOLUTION}_Range{int(GRID_MAX_RM)}RM_SP{ATOMS_PER_SUPERPARTICLE:.0e}_PSD_wide"
