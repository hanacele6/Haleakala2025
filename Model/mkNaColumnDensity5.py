...
def simulate_and_track_particle(args):
    ...
    tau = T1AU * AU ** 2
    # 粒子が地球の重力圏を大きく超えない程度の最大ステップ数を設定
    itmax = int((grid_params['R_MAX'] * 2 / 1000) / DT)  # おおよその最大時間
    ...
    for _ in range(itmax):
        ...
        if wl[0] * 1e-9 <= w_na_d2 < wl[-1] * 1e-9:
            ...
            F_lambda_1AU_m = JL * 1e4  # 単位を /m2/nm に
        ...
        # 合算した加速度
        total_accel_x = accel_gx - b  # 放射圧は-X方向
        total_accel_y = accel_gy
        total_accel_z = accel_gz

        # 速度と位置の更新 (Leapfrog法などの方が精度は良いが、単純なオイラー法で実装)
        ...
        if r <= RM:
            ...
            if np.random.random() < calculate_sticking_probability(temp_at_impact):
                ...
                death_reason = 'bounced'  # ここでは簡単化のため、跳ね返りは考慮せず消滅させる
            break
...
def main_time_evolution_with_density_output():
    ...
    N_LON = 24
    N_LAT = 24
    INITIAL_CNA_PER_M2 = 1.5e17  # [atoms/m^2]

    # ★★ 大気グリッド設定 ★★
    N_R = 50  # 半径方向の分割数
    N_THETA = 24  # 天頂角方向の分割数
    N_PHI = 24  # 方位角方向の分割数
    GRID_RADIUS_RM = 5.0  # グリッドの半径 (水星半径の何倍か)

    # ★★ 時間発展の設定 ★★
    TIME_STEP_SEC = 3600  # シミュレーションの1ステップの時間 [s]
    TOTAL_SIM_YEARS = 1.0  # 何水星年シミュレーションするか
    SAVE_INTERVAL_STEPS = 22  # 何ステップごとに結果を保存するか (1水星年=約88日=2112時間)

    # ★★ スーパーパーティクル設定 ★★
    # 1つのスーパーパーティクルが何個の実原子を表すか。計算負荷と精度に直結する重要パラメータ
    ...
    for step_idx, t_sec in enumerate(tqdm(time_steps, desc="Time Evolution")):
        ...
        for i_lon in range(N_LON):
            for i_lat in range(N_LAT):
                ...
                if cos_Z <= 0: continue  # 夜側なので放出なし

                # PSDによる放出率 [atoms/m^2/s]
                ...
                # 確率的に放出 (例: 2.7個なら2個は確定、残り1個は70%の確率で放出)
        ...
        # --- 3. 粒子追跡を実行 ---
        if not particles_to_launch_args:
            if (step_idx + 1) % SAVE_INTERVAL_STEPS == 0:  # 空でもファイルは作る
                empty_grid = np.zeros((N_R, N_THETA, N_PHI), dtype=np.float32)
                save_time_h = t_sec / 3600
                filename = f"atmospheric_density_t{int(save_time_h):05d}_taa{int(TAA):03d}.npy"
                np.save(os.path.join(target_output_dir, filename), empty_grid)
            continue

        with tqdm(total=len(time_steps), desc="Time Evolution", initial=step_idx) as pbar_main:
            if not particles_to_launch_args:
                # （粒子がない場合の処理は省略）...
                pbar_main.update(1)
                continue
            ...
            with Pool(processes=cpu_count()) as pool:
                # positionを指定することで、内側の進捗バーが2行目に表示される
                with tqdm(total=len(particles_to_launch_args), desc=f"Tracking TAA={TAA:.1f}", position=1,
                          leave=False) as pbar_particles:
                    results = []
                    for result in pool.imap(simulate_and_track_particle, particles_to_launch_args, chunksize=100):
                        results.append(result)
                        pbar_particles.update(1)
        ...
        if (step_idx + 1) % SAVE_INTERVAL_STEPS == 0:
            ...
            cell_volumes_m3 = delta_r3_3[:, np.newaxis, np.newaxis] * \
                              ...
                              (2 * np.pi / N_PHI)
            cell_volumes_m3[cell_volumes_m3 == 0] = 1e-30  # ゼロ除算を回避

            # 数密度 [atoms/m^3] を計算
            # (セル内の総滞在時間 * 1粒子あたりの原子数) / (セルの体積 * 1ステップの時間)
