...
def update_particles_numba(pos, vel, status_mask, result_codes, particle_bins, dt_step, V_rad, V_tan, r0, R_MAX, tau_ion, 
                           ...
                           u_model_type, sticking_model, hop_tau_ratio, use_prob_sticking, temp_base, temp_amp, temp_night, u_max, u_min):
...
def main_snapshot_simulation():
    ...
    run_name = f"HL_ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_{date_str}_{bd_str}_{u_str}_{q_str}_{a_str}_{lt_str}_15yr"
    # ==========================================================================
        ...
        #with Pool(cpu_count() - 1) as pool:
            ...
            while t_curr < t_end_run:  # 🌟 ここから時間ステップのループ
                ...
                if prev_taa > 350 and TAA < 10:  # 年越し判定 (近日点を通過)
                    current_year = int(round((t_curr - t_start_spinup) / MERCURY_YEAR))
                    if RECORD_HOP_TRANSPORT and (is_recording_phase or SAVE_SPINUP_PHASE):
                        hop_path = os.path.join(target_output_dir, f"hop_transport_hist_yr{current_year:02d}.npy")
                        np.save(hop_path, hop_hist)
                        print(f"[SAVE] 年越し: {current_year}年目のホップ輸送データを保存しました。")
                        hop_hist.fill(0.0)
