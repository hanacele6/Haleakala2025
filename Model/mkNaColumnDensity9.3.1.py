...
def simulate_particle_for_one_step(args: Dict) -> Dict:
    ...
    next_dt = settings['DT_MOVE']
    if time_remaining < 0.0:
        next_dt += time_remaining

    return {'status': 'alive', 'final_state': {'pos': pos, 'vel': vel, 'weight': weight, 'dt_remaining': next_dt}}
...
def main_snapshot_simulation():
    ...
    run_name = f"ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_0612_Multi_BD0.5_UG_Q2.0_Bouncetau30s_A2.0_LongLT(Fulle)"
                        ...
                        for j in range(N_LAT):
                            ...
                            for b in range(N_BINS_DYNAMIC):
                                ...
                                if b < N_BINS_DYNAMIC - 1:
                                    # 1. セルあたりの最大容量を計算 (1.0e15 atoms/cm^2 を m^2 に換算)
                                    # 1 cm^2 = 1e-4 m^2 なので、1.0e15 atoms/cm^2 = 1.0e19 atoms/m^2
                                    MAX_GLOBAL_DENSITY_M2 = 1.0e19
                                    
                                    # 次のビン(b+1)の許容量(キャパシティ)は、重み分布(V_weights)に比例する
                                    # ※ V_WEIGHTS_ARRAY が確率分布として生成されている前提
                                    bin_capacity_m2 = MAX_GLOBAL_DENSITY_M2 * V_WEIGHTS_ARRAY[b + 1]
                                    
                                    # 2. 次のビンが受け入れ可能な「空き容量 (dens)」を計算
                                    current_dens_next_bin = surface_density[i, j, b + 1]
                                    available_space_dens = max(0.0, bin_capacity_m2 - current_dens_next_bin)
                                    
                                    # 3. 実際に移動できる量を制限（クリッピング）
                                    actual_transfer_dens = min(transfer_dens, available_space_dens)
                                    
                                    # 4. 移動できなかった分（弾かれた分）は、元のビン(b)に戻す
                                    rejected_dens = transfer_dens - actual_transfer_dens
                                    if rejected_dens > 0:
                                        surface_density[i, j, b] += rejected_dens
                                        # (宇宙空間へのLossとしてカウントされているなら、ここで調整が必要になる場合がありますが、
                                        # 今回の transfer_dens は表面内での移動なので、単に元のセルに戻せばOKです)

                                    # 5. 制限された量だけを次のビンのソースとして追加
                                    supply_dens_total[b + 1] += actual_transfer_dens
                                    
                                    # 統計には実際に移動できた量だけを記録
                                    step_stats['Trans_Inward'] += actual_transfer_dens * cell_areas[j]
                    ...
                    for j in range(N_LAT):
                        ...
                        for b in range(N_BINS_DYNAMIC):
                            ...
                            for p_type, n_amount, T_or_none, w in params:
                                ...
                                # --- ゴースト化を防ぐための質量返還 ---
                                #actual_generated_amount = num * w
                                #unrepresented_amount = n_amount - actual_generated_amount
                                #if unrepresented_amount != 0:
                                    # 生成されなかった（または過剰に生成された）分の差額を、
                                    # surface_density に戻して次回のステップに持ち越す
                                #    surface_density[i, j, b] += unrepresented_amount / cell_areas[j]

                                #    if surface_density[i, j, b] < 0:
                                #        surface_density[i, j, b] = 0.0
