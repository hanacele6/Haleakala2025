...
日付: 2026/06/19 (Updated: Multi-Bin Model Integration)
==============================================================================
"""
...
def main_snapshot_simulation():
    ...
    # 2. 拡散モデル (BD) の判定
    # 'USE_STD_DIFFUSION' が True の場合のみ、活性化エネルギー(DIFF_E_A_EV)を反映
    if SIMULATION_SETTINGS.get('USE_STD_DIFFUSION', False):
        bd_str = f"BD{DIFF_E_A_EV:.1f}"
    else:
        bd_str = "NonBD"
        
    # 3. 束縛エネルギーモデル (UG / UF) と中心値の判定
    u_model = SIMULATION_SETTINGS.get('U_MODEL_TYPE', 'fixed')
    if u_model == 'gaussian_random':
        u_mu = SIMULATION_SETTINGS.get('U_GAUSSIAN_MU', 1.85)
        u_str = f"UG{u_mu:.2f}"
    elif u_model == 'fixed':
        # 固定値配列の最初の要素から取得
        u_fixed_arr = SIMULATION_SETTINGS.get('U_BINS_FIXED', np.array([1.85]))
        u_str = f"UF{u_fixed_arr[0]:.2f}"
    else:
        u_str = f"{u_model}" # dynamic等のフェイルセーフ用
        
    # 4. PSD放出効率 (Q) の判定 (cm^2 換算の係数部分を抽出)
    # 100**2 (m^2 -> cm^2換算) で割る前の、分子のベースとなる値を抽出して表記
    if u_model == 'fixed':
        q_bins_fixed = SIMULATION_SETTINGS.get('Q_PSD_BINS_FIXED', np.array([2.0e-20]))
        q_base = q_bins_fixed[0] * (100 ** 2)
    else:
    ...
    # 5. 拡散の基準流束 (A) の判定
    # 100**2 で割る前の値を基準に、10の指数表現にするか、数値をそのまま出すか
    # ここでは 2.0e7 * (100**2) の「2.0e7」の部分を取り出して A2.0e7 と表記
    a_base = DIFF_REF_FLUX / (100.0 ** 2)
    a_str = f"A{a_base:.1e}" # 例: A2.0e+07 (あるいはお好みで f"A{a_base/1e7:.1f}e7" -> A2.0e7)
    
    # 6. 電離寿命 (LT) の取得
    t1au_val = SIMULATION_SETTINGS.get('T1AU', 190000.0)
    # 万単位で分かりやすく表記 (例: 190000 -> LT190k)
    lt_str = f"LT{int(t1au_val // 1000)}k"
