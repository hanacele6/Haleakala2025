...
# ★★★ シミュレーション結果のディレクトリ
# output_dir = r"./SimulationResult_202511\DynamicGrid72x36_18.0"
...
if sim_taa is not None:
    ...
    if PLOT_MODE == "ALL":
        styles = {
            "DAWN": {"color": "blue", "marker": "^", "label": "Sim: Dawn"},
            "DUSK": {"color": "red", "marker": "v", "label": "Sim: Dusk"}
        }
        ...
        color_sim = 'blue'
        ax1.scatter(sim_taa, sim_data, label=f'Sim: {PLOT_MODE}', color=color_sim, alpha=0.8, s=50)
        if len(sim_data) > 0:
            y1_max_data = np.max(sim_data)

    ax1.set_xlabel('True Anomaly Angle (deg)', fontsize=14)
    ax1.set_ylabel(COMMON_Y_LABEL, fontsize=14, color='black')  # 左軸 (共通ラベル)
    ...
    if SHOW_CSV_OVERLAY:
        ...
        if not CSV_USE_SHARED_Y_AXIS:
            target_ax = ax1.twinx()
            target_ax.set_ylabel(COMMON_Y_LABEL, fontsize=14, color='black')  # 右軸 (共通ラベル)
        ...
        # --- 軸の同期処理 (右軸がある場合) ---
        # 修正: グリッド位置合わせを廃止し、余白比率のみを合わせるシンプルなロジック
        if not CSV_USE_SHARED_Y_AXIS and has_csv_plot:
            # 1. 共通のマージン（余白率）を決める (1.1 = 10%の余白)
            margin = 1.1

            # 2. それぞれのデータの最大値に合わせて上限を設定
            # データが存在する範囲の比率が揃うため、視覚的な振幅比較がしやすくなります
            # Matplotlibがそれぞれの軸でキリの良い数字を自動選択します
            ax1.set_ylim(bottom=0, top=y1_max_data * margin)
            target_ax.set_ylim(bottom=0, top=y2_max_data * margin)

            # 3. 右軸のグリッド線は非表示（左軸とズレて見にくくなるため）
