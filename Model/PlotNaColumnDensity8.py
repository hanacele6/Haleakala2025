...
# (Dawn全体 / Dusk全体)
NORMALIZATION_AREA_HALF_CM2 = NORMALIZATION_AREA_CM2
# (Dawn外側 / Dusk外側)
NORMALIZATION_AREA_QUARTER_CM2 = NORMALIZATION_AREA_CM2
...
#output_dir = r"./SimulationResult_202606/ParabolicHop_72x36_NoEq_DT100_0601_Multi_BD0.5_UG_Q2.0_Bouncetau30s_A1.0_LongLT(Fulle)"
...
def process_simulation_data(target_dir, mode, target_year):
    ...
    for filename, time_h, taa in tqdm(filtered_files, desc=f"Processing Year {target_year}"):
        ...
        # ==========================================================
        # 修正版: 3D空間全体から影を除外し、FOV(視野角)で切り取る処理
        # ==========================================================
        atoms_grid_3d = density_grid_m3 * cell_volume_m3

        # 1. 座標グリッドの生成 (X, Y, Z は中心が0のインデックス)
        X, Y, Z = np.meshgrid(
            np.arange(GRID_RESOLUTION) - mid_index_x,
            np.arange(GRID_RESOLUTION) - mid_index_y,
            np.arange(GRID_RESOLUTION) - mid_index_y,
            indexing='ij'
        )

        # 水星半径(1 RM)に相当するインデックス長
        RM_idx = (GRID_RESOLUTION - 1) / (2.0 * GRID_MAX_RM)
        rho_sq = Y**2 + Z**2

        # 2. 物理マスク：夜側 (X < 0) かつ 水星の影 (rho_sq <= RM_idx**2) を除外
        shadow_mask = (X < 0) & (rho_sq <= RM_idx**2)
        valid_atoms_3d = atoms_grid_3d * (~shadow_mask)

        # 3. 視線方向(X軸)に積分して、地球から見た2Dマップを作成 (形状: [Y, Z])
        atoms_2d = np.sum(valid_atoms_3d, axis=0)

        # 4. 観測FOVマスク：中心から 1.5 RM の範囲を切り取る
        CROP_RM = 1.5
        crop_limit = int(CROP_RM * RM_idx)
        # Y[0,:,:] と Z[0,:,:] で 2D平面の座標を取得し矩形枠を作成
        fov_mask = (np.abs(Y[0, :, :]) <= crop_limit) & (np.abs(Z[0, :, :]) <= crop_limit)
        
        atoms_2d_masked = atoms_2d * fov_mask

        # 5. 共通部分和 (Y軸の中心で分割)
        sum_mid = 0
        if mode in ["DAWN", "DUSK", "ALL"]:
            sum_mid = np.sum(atoms_2d_masked[mid_index_y, :])

        if mode == "ALL":
            # DAWN (Y < 0 の領域)
            sum_dawn = np.sum(atoms_2d_masked[:mid_index_y, :])
            ...
            # DUSK (Y > 0 の領域)
            sum_dusk = np.sum(atoms_2d_masked[mid_index_y + 1:, :])
            ...
            if mode == "DAYSIDE_TOTAL":
                total_atoms = np.sum(atoms_2d_masked)
                ...
                sum_dawn_outer = np.sum(atoms_2d_masked[:idx_dawn_outer_limit, :])
                total_atoms = sum_dawn_outer
                target_area = NORMALIZATION_AREA_QUARTER_CM2
            elif mode == "DUSK_OUTER":
                sum_dusk_outer = np.sum(atoms_2d_masked[idx_dusk_outer_start:, :])
