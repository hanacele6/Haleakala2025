...
def main_time_evolution_with_density_output():
    ...
    run_name = f"PSD_atm_R{N_R}_YEAR{TOTAL_SIM_YEARS:.1f}_SP{ATOMS_PER_SUPERPARTICLE:.0e}_Q2.0_new"
    ...
    with tqdm(total=len(time_steps), desc="Time Evolution") as pbar:
        for step_idx, t_sec in enumerate(time_steps):
            ...
            # 359度 -> 0度のようにTAAがラップアラウンドした場合の処理
            if TAA < previous_taa:
                target_taa_idx = 0  # 次のターゲットを最初に戻す
            ...
            previous_taa = TAA  # ループの最後に現在のTAAを保存
            pbar.update(1)
