...
import numba
# 'tbb' (Intel Threading Building Blocks) がインストールされていれば最速ですが、
# なければ標準の 'workqueue' を指定します。
...
def evaluate_baule_collision(vel_in: np.ndarray, normal: np.ndarray, U_0_eV: float, mass_target_amu: float = 16.0) -> Tuple[bool, np.ndarray]:
    """
    Zangwill/Bauleモデルに基づく表面衝突・エネルギー散逸判定。
    戻り値: (is_trapped: bool, vel_out: np.ndarray)
    """
    # 質量比 (Na:22.99) と エネルギー伝達係数
    m_Na_amu = 22.99
    mu = m_Na_amu / mass_target_amu
    transfer_coeff = (4 * mu) / ((1 + mu)**2)
    
    # 法線方向(n)と接線方向(t)の速度ベクトル分離
    v_n_scalar = np.dot(vel_in, normal)  # 表面に向かうため負
    v_n_vec = v_n_scalar * normal
    v_t_vec = vel_in - v_n_vec
    
    # 法線方向の運動エネルギー [eV]
    v_n_mag = np.abs(v_n_scalar)
    E_n_eV = (0.5 * PHYSICAL_CONSTANTS['MASS_NA'] * (v_n_mag**2)) / PHYSICAL_CONSTANTS['EV_TO_JOULE']
    
    # 吸着ポテンシャルの井戸の底での総エネルギー (E_impact = E_n + U_0)
    E_impact = E_n_eV + U_0_eV
    
    # フォノンへ散逸するエネルギー
    delta_E = transfer_coeff * E_impact
    
    if delta_E >= E_n_eV:
        # 散逸エネルギーが元の垂直運動エネルギーを上回る -> 井戸にトラップされる
        return True, np.zeros(3)
    else:
        # トラップされず、残存エネルギーを持って弾き返される(非弾性散乱)
        E_rem_n = E_n_eV - delta_E
        v_rem_n_mag = np.sqrt((2 * E_rem_n * PHYSICAL_CONSTANTS['EV_TO_JOULE']) / PHYSICAL_CONSTANTS['MASS_NA'])
        
        # 表面から外へ向かう法線ベクトル + 保存された接線成分
        v_out_n_vec = v_rem_n_mag * normal 
        vel_out = v_out_n_vec + v_t_vec
        return False, vel_out
    

# ==============================================================================
# 2. 粒子運動計算・衝突エンジン (Numba完全対応版 / SoAアーキテクチャ)
# ==============================================================================
...
def assign_sticking_bin_numba(E_in_eV, temp_impact_K, U_bins, V_weights, u_model_type, u_max, u_min):
    ...
    if u_model_type == 0: # 'fixed'
        ...
        probs = np.zeros(10) # N_U_BINS=10を固定配列として確保
        p_sum = 0.0
        for b in range(n_bins):
            energy_factor = np.exp(- ((U_bins[b] - U_target)**2) / (kBT_eV * gamma_val))
            probs[b] = V_weights[b] * energy_factor
            p_sum += probs[b]
...
def calculate_acceleration_numba(px, py, pz, vx, vy, vz, V_rad, V_tan, r0, spec_wl, spec_gamma, JL_const, AU):
    ...
    ax = accel_g_x + accel_cen_x + accel_cor_x + accel_srp_x
    ay = accel_g_y + accel_cen_y + accel_cor_y + accel_srp_y
    az = accel_g_z + accel_cen_z + accel_cor_z + accel_srp_z
...
def update_particles_numba(pos, vel, status_mask, result_codes, particle_bins, dt_step, V_rad, V_tan, r0, R_MAX, tau_ion, 
                           ...
                           random_vals): # random_vals を引数に残す
    """
    完全版：SoA/Numba 粒子移動・衝突・マルチバウンド一括処理エンジン
    (チャンク分割による完全ロックフリー並列化)
    """
    ...
    for chunk in prange(n_chunks):
        ...
        # 🌟 チャンクの内部は通常の for ループ（これでNumbaが安心する）
        for i in range(start, end):
            ...
            while t_rem > 1e-6:
                ...
                # 1. 光電離判定 (昼側のみ)
                # 🌟 事前生成した random_vals を使う
                if px > 0 and random_vals[i] < (1.0 - np.exp(-dt / tau_ion)):
                ...
                if r_next_sq <= RM_NB**2:
                    ...
                    if sticking_model == 1: # 'baule'
                        ...
                        if use_prob_sticking:
                            # 🌟 ここも random_vals[i] に変更！
                            if random_vals[i] < calculate_sticking_probability_nb(temp_impact):
                                is_trapped = True
                        else:
...
@njit(fastmath=True)
def deposit_stuck_particles_numba(pos, weight, result_codes, particle_bins,
                                  gained_grid, subsolar_lon,
                                  n_lon, n_lat, n_bins):
    """
    Stuckした粒子を表面密度グリッドへ戻すNumba版。
    注意:
        parallel=Trueにはしない。
        gained_grid[ix, iy, b] += w に競合が起きるため。
    """
    ...
    for i in range(pos.shape[0]):
        ...
        b = particle_bins[i]
        if 0 <= b < n_bins:
            gained_grid[ix, iy, b] += weight[i]


@njit(fastmath=True)
def generate_particles_numba(
    pos_out, vel_out, weight_out, status_mask,
    empty_idx, max_particles,  # start_idx を empty_idx配列 に変更
    lon_edges, lat_edges, subsolar_lon,
    total_loss_step, frac_psd, frac_td, frac_sws,
    TEMP_PSD, TEMP_NIGHT, w_psd, w_td, w_sws,
    n_lon, n_lat, n_bins, RM
):
    ptr = 0  # empty_idx を参照するためのポインタ
    n_empty = empty_idx.shape[0]
    ...
    for i in range(n_lon):
        ...
        for j in range(n_lat):
            ...
            nx = np.cos(lat_f) * np.cos(lon_f - subsolar_lon)
            ny = np.cos(lat_f) * np.sin(lon_f - subsolar_lon)
            nz = np.sin(lat_f)

            px0, py0, pz0 = nx * RM, ny * RM, nz * RM
            ...
            for b in range(n_bins):
                ...
                if n_psd > 0.0:
                    num_f = n_psd / w_psd
                    num = int(num_f)
                    if np.random.random() < (num_f - num):
                        num += 1

                    for _ in range(num):
                        if ptr >= n_empty: break # 空き容量が尽きたら終了
                        idx = empty_idx[ptr]     # 確実な空きインデックスを取得

                        lx, ly, lz = sample_lambertian_direction_numba()
                        spd = sample_speed_from_flux_numba(TEMP_PSD)
                        vx, vy, vz = transform_local_to_world_numba(lx, ly, lz, nx, ny, nz)

                        pos_out[idx, 0], pos_out[idx, 1], pos_out[idx, 2] = px0, py0, pz0
                        vel_out[idx, 0], vel_out[idx, 1], vel_out[idx, 2] = vx * spd, vy * spd, vz * spd
                        weight_out[idx] = w_psd
                        status_mask[idx] = True
                ...
                if n_td > 0.0:
                    ...
                    for _ in range(num):
                        ...
                        lx, ly, lz = sample_lambertian_direction_numba()
                        spd = sample_speed_from_flux_numba(TEMP_NIGHT)
                        vx, vy, vz = transform_local_to_world_numba(lx, ly, lz, nx, ny, nz)
                ...
                if n_sws > 0.0:
                    ...
                    for _ in range(num):
                        ...
                        lx, ly, lz = sample_lambertian_direction_numba()
                        E = 1.0  
                        spd = np.sqrt(2.0 * E * 1.602e-19 / 3.8175e-26) # EV_TO_JOULE_NB / MASS_NA_NB
                        vx, vy, vz = transform_local_to_world_numba(lx, ly, lz, nx, ny, nz)

                        pos_out[idx, 0], pos_out[idx, 1], pos_out[idx, 2] = px0, py0, pz0
                        vel_out[idx, 0], vel_out[idx, 1], vel_out[idx, 2] = vx * spd, vy * spd, vz * spd
                        weight_out[idx] = w_sws
                        status_mask[idx] = True
                        
                        total_gen_sws += w_sws
                        ptr += 1

    return total_gen_psd, total_gen_td, total_gen_sws
    ...
    while time_remaining > 1e-6:
        ...
        if pos[0] > 0 and np.random.random() < (1.0 - np.exp(-dt_this_loop / tau_ion)):
            ...
            if altitude < 10000.0:
                ...
                E_eV = (0.5 * PHYSICAL_CONSTANTS['MASS_NA'] * v_mag**2) / PHYSICAL_CONSTANTS['EV_TO_JOULE']
                # 落下地点の経度・緯度を計算して温度を出す
                lon_rot_ion = np.arctan2(pos[1], pos[0])
                lat_rot_ion = np.arcsin(np.clip(pos[2] / PHYSICAL_CONSTANTS['RM'], -1, 1))
                lon_fixed_ion = (lon_rot_ion + subsolar_lon + PHYSICAL_CONSTANTS['PI']) % (2 * PHYSICAL_CONSTANTS['PI']) - PHYSICAL_CONSTANTS['PI']
                temp_ion_impact = calculate_surface_temperature_leblanc(lon_fixed_ion, lat_rot_ion, AU, subsolar_lon)

                bin_idx = assign_sticking_bin(E_eV, temp_ion_impact, U_bins, V_weights, settings)                
                return {'status': 'stuck', 'pos_at_impact': pos, 'weight': weight, 'bin_idx': bin_idx}
            else:
                return {'status': 'ionized', 'weight': weight}
        ...
        if is_hit:
            ...
            v_hit_mag = np.linalg.norm(vel_hit_in)
            E_hit_eV = (0.5 * PHYSICAL_CONSTANTS['MASS_NA'] * v_hit_mag**2) / PHYSICAL_CONSTANTS['EV_TO_JOULE']

            bin_idx = assign_sticking_bin(E_hit_eV, temp_impact, U_bins, V_weights, settings)
            U_assigned = U_bins[bin_idx] 
            norm_hit = pos_hit / RM

            is_trapped = False
            vel_rebound = np.zeros(3)

            # ==========================================================
            # 1. 吸着モデルの選択と判定
            # ==========================================================
            sticking_model = settings.get('STICKING_MODEL', 'empirical')

            if sticking_model == 'baule':
                # 【モデルA】Zangwill / Baule による力学的な散逸判定
                is_trapped, vel_rebound = evaluate_baule_collision(
                    vel_in=vel_hit_in, 
                    normal=norm_hit, 
                    U_0_eV=U_assigned, 
                    mass_target_amu=16.0
                )
            else:
                # 【モデルB】経験的モデル または 300K閾値モデル
                if settings.get('USE_PROBABILISTIC_STICKING', False):
                    # 確率的吸着
                    prob = calculate_sticking_probability(temp_impact)
                ...
                # 経験的・閾値モデルで「弾かれた」場合の熱的バウンド処理
                if not is_trapped:
                    spd = sample_speed_from_flux_distribution(PHYSICAL_CONSTANTS['MASS_NA'], temp_impact)
                    vel_rebound = spd * transform_local_to_world(sample_lambertian_direction_local(), norm_hit)

            # ==========================================================
            # 2. 弾かれた（トラップされなかった）場合の即時バウンド
            # ==========================================================
            if not is_trapped:
                pos = (RM + 1.0) * norm_hit
                vel = vel_rebound
            ...
            if tau_td <= HOP_TAU_THRESHOLD and time_remaining > tau_td:
                ...
                # 滞在時間が長い、あるいは残り時間が足りない場合は一旦吸着(Stuck)
...
def main_snapshot_simulation():
    ...
    OUTPUT_DIRECTORY = r"./SimulationResult_202605"
    ...
    run_name = f"ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_0529_Multi_BD0.5_UG_Q2.0_Bouncetau30s_A1.0_LongLT(Fulle)"
    ...
    if total_cores and total_cores > 1:
        use_threads = total_cores - 1
        numba.set_num_threads(use_threads)
        print(f"Parallel computing enabled. Using {use_threads} / {total_cores} CPU cores (1 core reserved for OS/IO).")
    else:
        numba.set_num_threads(1)
        print("Single core execution enforced.")
        ...
        #with Pool(cpu_count() - 1) as pool:
            ...
            while t_curr < t_end_run:  # 🌟 ここから時間ステップのループ
                ...
                if time_since_last_update >= DT_RATE_UPDATE:
                    ...
                    # 1. 経度・緯度グリッドの作成 (2次元配列)
                    # lon_edges, lat_edges から中心座標を計算
                    lon_centers = (lon_edges[:-1] + lon_edges[1:]) / 2.0
                    lat_centers = (lat_edges[:-1] + lat_edges[1:]) / 2.0
                    
                    # np.meshgrid で 2次元グリッド (N_LON x N_LAT) を作成
                    # indexing='ij' で (i, j) へのアクセス順を維持
                    LON_GRID, LAT_GRID = np.meshgrid(lon_centers, lat_centers, indexing='ij')

                    # 2. 角度(cos_z) と 照度フラクション(illum_frac) の一括計算
                    cos_z_center = np.cos(LAT_GRID) * np.cos(LON_GRID - sub_lon)
                    
                    illum_frac = np.zeros_like(cos_z_center)
                    # 半影領域の計算
                    mask_full_day = cos_z_center > sin_half_width
                    mask_full_night = cos_z_center < -sin_half_width
                    mask_terminator = ~(mask_full_day | mask_full_night)
                    
                    illum_frac[mask_full_day] = 1.0
                    illum_frac[mask_terminator] = (cos_z_center[mask_terminator] + sin_half_width) / (2 * sin_half_width)
                    # 夜側は 0.0 のまま

                    if not SIMULATION_SETTINGS['USE_AREA_WEIGHTED_FLUX'] and not SIMULATION_SETTINGS['USE_SUBGRID_SMOOTHING']:
                        illum_frac = np.where(cos_z_center > 0, 1.0, 0.0)

                    eff_cos = np.maximum(0.0, cos_z_center)
                    ...
                    # 4. 内部拡散 (Std Diffusion) の一括計算
                    supply_dens_std_diff = np.zeros_like(T_day_potential)
                    if SIMULATION_SETTINGS['USE_STD_DIFFUSION']:
                        mask_hot = T_day_potential > 100.0
                        flux_val = DIFF_PRE_FACTOR * np.exp(-DIFF_E_A_EV / (KB_EV_CONST * T_day_potential[mask_hot]))
                        supply_dens_std_diff[mask_hot] = flux_val * dt_accumulated

                    # Clamped Diffusion, Forced Injection は現状0.0として省略（必要なら同様にマスクで実装可能）
                    supply_dens_forced = np.zeros_like(T_day_potential)
                    # (必要ならここにForced Injectionのベクトル処理を記述)

                    total_supply_amount_grid = supply_dens_std_diff + supply_dens_forced

                    # 【統計】地殻からの新規供給(内部拡散)を記録
                    # cell_areas は 1次元配列(N_LAT) なのでブロードキャストして面積を掛ける
                    step_stats['Supply_Internal'] += np.sum(total_supply_amount_grid * cell_areas)

                    # 5. 放出率マップの3次元化と計算 (N_LON, N_LAT, N_BINS)
                    ...
                    # SWSのマスク作成
                    lon_sun = (LON_GRID - sub_lon + np.pi) % (2 * np.pi) - np.pi
                    mask_sws_lon = (SWS_PARAMS['LON_RANGE'][0] <= lon_sun) & (lon_sun <= SWS_PARAMS['LON_RANGE'][1])
                    mask_sws_lat = ((SWS_PARAMS['LAT_N_RANGE'][0] <= LAT_GRID) & (LAT_GRID <= SWS_PARAMS['LAT_N_RANGE'][1])) | \
                                   ((SWS_PARAMS['LAT_S_RANGE'][0] <= LAT_GRID) & (LAT_GRID <= SWS_PARAMS['LAT_S_RANGE'][1]))
                    mask_sws = mask_sws_lon & mask_sws_lat


                    diffusion_ratio = get_diffusion_supply_distribution(1.0, SIMULATION_SETTINGS['U_BINS'], SIMULATION_SETTINGS)

                    for b in range(N_BINS_DYNAMIC):
                        # 内部拡散のビンへの分配 (スカラ関数をベクトル適用)
                        # get_diffusion_supply_distribution の結果は (N_BINS,) の配列
                        # ここでは単一ビンの供給量スカラーを計算するのではなく、グリッド全体に掛ける係数を取得します。
                        # ※注意: get_diffusion_supply_distribution は「1単位の供給がどう分配されるか」の比率を返すように内部ロジックを読み替えます。
                        # つまり `total_supply_amount_grid * distribution_ratio[b]` とします。
                    
                        supply_dens_total_b = total_supply_amount_grid * diffusion_ratio[b]

                        # PSD
                        # SIMULATION_SETTINGS['Q_PSD_BINS'][b] はスカラー
                        temp_rate_psd[:, :, b] = f_uv * SIMULATION_SETTINGS['Q_PSD_BINS'][b] * eff_cos * illum_frac

                        # TD (Thermal Desorption) のベクトル計算
                        # calculate_thermal_desorption_rate を Numpy で一括処理できるようにインライン展開
                        U_JOULE = SIMULATION_SETTINGS['U_BINS'][b] * PHYSICAL_CONSTANTS['EV_TO_JOULE']
                        exponent_day = -U_JOULE / (PHYSICAL_CONSTANTS['K_BOLTZMANN'] * T_day_potential)
                        rate_day = np.where(exponent_day >= -700, 1e13 * np.exp(exponent_day), 0.0)
                        rate_day[T_day_potential < 10.0] = 0.0
                        
                        if SIMULATION_SETTINGS['USE_AREA_WEIGHTED_FLUX']:
                            # 夜側温度でのレート
                            exponent_night = -U_JOULE / (PHYSICAL_CONSTANTS['K_BOLTZMANN'] * SIMULATION_SETTINGS['TEMP_NIGHT'])
                            rate_night = 1e13 * np.exp(exponent_night) if exponent_night >= -700 else 0.0
                            temp_rate_td[:, :, b] = rate_day * illum_frac + rate_night * (1.0 - illum_frac)
                        else:
                            # illum_frac > 0.5 の場所は rate_day, そうでない場所は夜間レート(ほぼ0)とする
                            exponent_night = -U_JOULE / (PHYSICAL_CONSTANTS['K_BOLTZMANN'] * SIMULATION_SETTINGS['TEMP_NIGHT'])
                            rate_night = 1e13 * np.exp(exponent_night) if exponent_night >= -700 else 0.0
                            temp_rate_td[:, :, b] = np.where(illum_frac > 0.5, rate_day, rate_night)

                        # SWS
                        sws_val = (sw_flux * SWS_PARAMS['YIELD']) / SWS_PARAMS['REF_DENS']
                        temp_rate_sws[:, :, b] = np.where(mask_sws, sws_val, 0.0)

                        # 宇宙空間への喪失レート合計
                        rate_total = temp_rate_psd[:, :, b] + temp_rate_td[:, :, b] + temp_rate_sws[:, :, b]
                        
                        current_dens = surface_density[:, :, b]
                        # accumulated_gained_grid (吸着によって増えた分) は cell_areas で割る
                        # broadcasting: (N_LON, N_LAT) / (N_LAT,) -> (N_LON, N_LAT)
                        gain_dens = accumulated_gained_grid[:, :, b] / cell_areas
                        total_input_dens = gain_dens + supply_dens_total_b

                        transfer_rate = np.zeros_like(T_day_potential)
                        if SIMULATION_SETTINGS.get('USE_SURFACE_DIFFUSION_TRAPPING', False) and (b < N_BINS_DYNAMIC - 1):
                            mask_transfer = T_day_potential > 10.0
                            transfer_rate[mask_transfer] = SIMULATION_SETTINGS['A_DIFF_RATE'] * \
                                                           np.exp(-SIMULATION_SETTINGS['E_DIFF_EV'] / (KB_EV_CONST * T_day_potential[mask_transfer]))

                        effective_total_rate = rate_total + transfer_rate
                        
                        # 0割りを防ぐためのマスク
                        mask_rate_positive = effective_total_rate > 1e-30
                        
                        # 非定常状態 (Analytical Decay) の一括計算
                        decay_factor = np.ones_like(effective_total_rate)
                        decay_factor[mask_rate_positive] = np.exp(-effective_total_rate[mask_rate_positive] * dt_accumulated)
                        
                        total_loss_dens = current_dens * (1.0 - decay_factor)
                        
                        actual_loss_dens = np.zeros_like(total_loss_dens)
                        transfer_dens = np.zeros_like(total_loss_dens)
                        
                        actual_loss_dens[mask_rate_positive] = total_loss_dens[mask_rate_positive] * (rate_total[mask_rate_positive] / effective_total_rate[mask_rate_positive])
                        transfer_dens[mask_rate_positive] = total_loss_dens[mask_rate_positive] * (transfer_rate[mask_rate_positive] / effective_total_rate[mask_rate_positive])

                        # 密度の更新
                        surface_density[:, :, b] = np.maximum(0.0, (current_dens - total_loss_dens) + total_input_dens)
                        
                        # 宇宙空間への喪失量（パーティクル生成用）
                        temp_loss_per_sec[:, :, b] = actual_loss_dens * cell_areas / dt_accumulated

                        # === 次のビン(より深いサイト)への移動と「飽和(Saturation)」の制限 ===
                        if b < N_BINS_DYNAMIC - 1:
                            MAX_GLOBAL_DENSITY_M2 = 1.0e19
                            bin_capacity_m2 = MAX_GLOBAL_DENSITY_M2 * V_WEIGHTS_ARRAY[b + 1]
                            
                            current_dens_next_bin = surface_density[:, :, b + 1]
                            available_space_dens = np.maximum(0.0, bin_capacity_m2 - current_dens_next_bin)
                            
                            actual_transfer_dens = np.minimum(transfer_dens, available_space_dens)
                            
                            rejected_dens = transfer_dens - actual_transfer_dens
                            surface_density[:, :, b] += rejected_dens
                            
                            # 制限された量だけを次のビンのソースとして追加 (次のループの total_input_dens に加算されるよう、gain_dens 相当の場所に積む)
                            # ここでは直接 accumulated_gained_grid の次ビンに追加しておく（面積を掛けて戻す）
                            accumulated_gained_grid[:, :, b + 1] += actual_transfer_dens * cell_areas
                            
                            step_stats['Trans_Inward'] += np.sum(actual_transfer_dens * cell_areas)
                ...
                if len(empty_idx) > 0:
                    gen_stats = generate_particles_grid_numba(
                        ...
                        T_day_potential, illum_frac, lon_edges, lat_edges, sub_lon, PHYSICAL_CONSTANTS['RM']
                    )
                ...
                if n_mmv > 0:
                    num_p = int(n_mmv / w_mmv)
                    if np.random.random() < (n_mmv / w_mmv - num_p):
                        num_p += 1
                ...
                if prev_taa != -999:
                    ...
                    if passed:
                        ...
                        timeseries_log.append({
                            ...
                            'Gen_PSD_Dawn': step_stats['Gen_PSD_Dawn'], 'Gen_PSD_Dusk': step_stats['Gen_PSD_Dusk'],
                            'Gen_TD_Dawn': step_stats['Gen_TD_Dawn'], 'Gen_TD_Dusk': step_stats['Gen_TD_Dusk'],
                            'Gen_SWS_Dawn': step_stats['Gen_SWS_Dawn'], 'Gen_SWS_Dusk': step_stats['Gen_SWS_Dusk'],
                            'Gen_MMV_Dawn': step_stats['Gen_MMV_Dawn'], 'Gen_MMV_Dusk': step_stats['Gen_MMV_Dusk'],
                        ...
                        # status_mask が True (生きている) 粒子のデータだけを抽出
                        pos_arr = pos_array[status_mask]
                        weights_arr = weight_array[status_mask]
