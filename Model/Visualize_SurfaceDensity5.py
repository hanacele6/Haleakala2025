...
class SimulationViewer:
    def __init__(self, grouped_files, orbit_data, t_start):
        ...
        plt.subplots_adjust(bottom=0.25, left=0.2, right=0.95)
        ...
        self.ax_prev = plt.axes([0.21, 0.05, 0.08, 0.05])
        self.btn_prev = Button(self.ax_prev, 'Previous')
        self.btn_prev.on_clicked(self.prev_frame)

        self.ax_next = plt.axes([0.30, 0.05, 0.08, 0.05])
        self.btn_next = Button(self.ax_next, 'Next')
        self.btn_next.on_clicked(self.next_frame)

        self.ax_mode = plt.axes([0.39, 0.05, 0.12, 0.05])
        self.btn_mode = Button(self.ax_mode, f'Mode: {self.step_mode}', color='lightblue')
        self.btn_mode.on_clicked(self.toggle_step_mode)

        self.ax_plate = plt.axes([0.52, 0.05, 0.10, 0.05])
        self.btn_plate = Button(self.ax_plate, 'Show Plate')
        self.btn_plate.on_clicked(self.generate_plate_view)

        self.ax_total = plt.axes([0.63, 0.05, 0.10, 0.05])
        self.btn_total = Button(self.ax_total, 'Plot Total')
        self.btn_total.on_clicked(self.plot_total_atoms)

        self.ax_gif = plt.axes([0.02, 0.22, 0.15, 0.05])
        self.btn_gif = Button(self.ax_gif, 'Save GIF (Current Mode)', color='salmon')
        self.btn_gif.on_clicked(self.on_gif_btn_clicked)

        self.ax_check = plt.axes([0.74, 0.02, 0.22, 0.12])
        self.check = CheckButtons(
            self.ax_check, 
            ['Contours', 'SSP', 'Terminator', 'Planet 0°'], 
            [self.show_contours, self.show_ssp, self.show_terminator, self.show_planet_zero]
        )
    ...
    def update_plot(self):
        ...
        if self.align_sun:
            if self.show_ssp:
                self.ax.axvline(0.0, color='#FFBF00', linestyle='-', linewidth=2.0, alpha=0.8, label='SSP (Fixed)')
            if self.show_terminator:
                self.ax.axvline(-90.0, color='white', linestyle='--', linewidth=1.5, alpha=0.7, label='Terminator (Dawn, Fixed)')
                self.ax.axvline(90.0, color='white', linestyle='--', linewidth=1.5, alpha=0.7, label='Terminator (Dusk, Fixed)')
            
            planet_zero_disp = (-ssp_planet_lon + 180) % 360 - 180
            if self.show_planet_zero:
                self.ax.axvline(planet_zero_disp, color='lime', linestyle='-.', linewidth=2.0, alpha=0.8, label='Planet 0°')
            
            if self.show_terminator:
                moving_dawn = (-ssp_planet_lon - 90 + 180) % 360 - 180
                moving_dusk = (-ssp_planet_lon + 90 + 180) % 360 - 180
                self.ax.axvline(moving_dawn, color='cyan', linestyle=':', linewidth=1.2, alpha=0.6, label='Moving Term (Dawn)')
                self.ax.axvline(moving_dusk, color='cyan', linestyle=':', linewidth=1.2, alpha=0.6, label='Moving Term (Dusk)')
        else:
            if self.show_planet_zero:
                self.ax.axvline(0.0, color='lime', linestyle='-.', linewidth=2.0, alpha=0.8, label='Planet 0° (Fixed)')
            if self.show_ssp:
                self.ax.axvline(ssp_planet_lon, color='#FFBF00', linestyle='-', linewidth=2.0, alpha=0.8, label='SSP (Moving)')
            if self.show_terminator:
                term_dawn_disp = (ssp_planet_lon - 90 + 180) % 360 - 180
                term_dusk_disp = (ssp_planet_lon + 90 + 180) % 360 - 180
                self.ax.axvline(term_dawn_disp, color='white', linestyle='--', linewidth=1.5, alpha=0.7, label='Terminator (Dawn, Moving)')
                self.ax.axvline(term_dusk_disp, color='white', linestyle='--', linewidth=1.5, alpha=0.7, label='Terminator (Dusk, Moving)')
    ...
    def _get_next_index_by_mode(self, start_idx):
        if self.step_mode == "TAA":
            # ファイル自体がTAA刻みなので、インデックスを+1するだけでTAAが1度進む
            return min(start_idx + 1, len(self.file_list) - 1)
        else:
            # 等時間（例: 24時間）経過したファイルを探す
