...
SIMULATION_SETTINGS = {
    ...
    'USE_PROBABILISTIC_STICKING': False, # 確率的吸着か閾値(300K)吸着かの切り替え
    ...
    # 拡散供給時の束縛エネルギー設定
    'DIFFUSION_U_MODE': 'single',  # 'single'(単一ビン) または 'gaussian'(ガウス分布)
    'DIFFUSION_U_TARGET': 2.7,     # singleの場合：どの深さに供給するか [eV]
    ...
    'USE_SURFACE_DIFFUSION_TRAPPING': True, # オンオフ機構 (Trueで深いビンへ移動)
...
# 3. メインルーチン
# ==============================================================================
def main_snapshot_simulation(custom_settings, target_output_dir, run_name):

    sim_settings = custom_settings
    
    t_end_run = sim_settings['T_END_RUN'] # 1年で終了
    ...
    OUTPUT_DIRECTORY = r"./SimulationResult_202604"
    ...
    run_name = f"ParabolicHop_{N_LON_FIXED}x{N_LAT}_{mode_str}_DT{int(DT_MOVE)}_0428_Multi_BD0.4SD0.6,4e3_U1.85_Q0.27_Bouncetau30s_A0.5_LongLT"
    ...
    try:  # 🌟 Ctrl+Cをキャッチするための開始地点
        ...
        with Pool(cpu_count() - 1) as pool:
            ...
            while t_curr < t_end_run:  # 🌟 ここから時間ステップのループ
                ...
                if time_since_last_update >= DT_RATE_UPDATE:
                    ...
                    for i in range(N_LON_FIXED):
                        for j in range(N_LAT):
                            ...
                            total_supply_amount = supply_dens_std_diff + supply_dens_clamped + supply_dens_forced
                            
                            # ====================================================================
                            # 【追加1】地殻からの新規供給(内部拡散)を記録
                            # total_supply_amount はまだビン分配前の「そのセルの総供給密度」です。
                            # これにセル面積を掛けて、純粋な新規供給の「個数」として計上します。
                            # ====================================================================
                            step_stats['Supply_Internal'] += total_supply_amount * cell_areas[j]

                            supply_dens_total = get_diffusion_supply_distribution(
                                total_supply_amount, 
                                SIMULATION_SETTINGS['U_BINS'], 
                                SIMULATION_SETTINGS
                            )
                            ...
                            for b in range(N_BINS_DYNAMIC):
                                ...
                                if SIMULATION_SETTINGS.get('USE_SURFACE_DIFFUSION_TRAPPING', False) and (b < N_BINS_DYNAMIC - 1):
                                    if T_day_potential > 10.0:
                                        transfer_rate = SIMULATION_SETTINGS['A_DIFF_RATE'] * \
                                                        np.exp(-SIMULATION_SETTINGS['E_DIFF_EV'] / (KB_EV_CONST * T_day_potential))

                                # 宇宙空間への喪失(rate_total)と深いビンへの喪失(transfer_rate)を合算
                                effective_total_rate = rate_total + transfer_rate
                                timescale = 1.0 / effective_total_rate if effective_total_rate > 1e-30 else float('inf')
                                allow_eq_mode = (step_count > 1)

                                if SIMULATION_SETTINGS['USE_EQUILIBRIUM_MODE'] and allow_eq_mode and (timescale <= dt_accumulated):
                                    # --- 定常状態 (Equilibrium Mode) ---
                                    if effective_total_rate > 1e-30:
                                        dens_eq = (total_input_dens / dt_accumulated) / effective_total_rate
                                    else:
                                        dens_eq = current_dens + total_input_dens
                                    
                                    surface_density[i, j, b] = dens_eq
                                    total_loss_dens = total_input_dens # 定常状態では入力がそのまま全喪失量となる
                                    
                                    if effective_total_rate > 1e-30:
                                        actual_loss_dens = total_loss_dens * (rate_total / effective_total_rate)
                                        transfer_dens = total_loss_dens * (transfer_rate / effective_total_rate)
                                    else:
                                        actual_loss_dens = total_loss_dens
                                        transfer_dens = 0.0
                                else:
                                    # --- 非定常状態 (Analytical Decay) ---
                                    decay_factor = np.exp(-effective_total_rate * dt_accumulated)
                                    ...
                                    if effective_total_rate > 1e-30:
                                        ...
                                        actual_loss_dens = 0.0
                                        transfer_dens = 0.0
                                        
                                    surface_density[i, j, b] = (current_dens - total_loss_dens) + total_input_dens

                                if surface_density[i, j, b] < 0: surface_density[i, j, b] = 0
                                
                                # 外気圏への放出対象となる量のみを登録（パーティクル生成用）
                                temp_loss_per_sec[i, j, b] = actual_loss_dens * cell_areas[j] / dt_accumulated

                                # === 次のビン(より深いサイト)のソースとして追加 ===
                                if b < N_BINS_DYNAMIC - 1:
                                    supply_dens_total[b + 1] += transfer_dens
                                    # ====================================================================
                                    # 【追加2】表面拡散による深いビンへの移動量 (Trans_Inward) を記録
                                    # transfer_dens (移動した密度) にセル面積を掛けて個数とし、加算します。
                                    # ====================================================================
                                    step_stats['Trans_Inward'] += transfer_dens * cell_areas[j]
                ...
                if tasks:
                    # 🌟 chunksize=500 で通信渋滞を回避！
                    results = pool.map(simulate_particle_for_one_step, tasks, chunksize=500)
...
def run_automated_comparisons():
    # ベースとなる共通設定（現在のSIMULATION_SETTINGSなどを想定）
    # ※シミュレーション期間を「1年分」の秒数（水星なら約88日分など）に設定してください
    ONE_YEAR_SECONDS = 88.0 * 24 * 3600  # 例: 水星の1年
    
    scenarios = [
        {
            "name": "1_fixed_U2.7_no_prob_sticking",
            "updates": {
                'U_DIST_MODE': 'fixed',            # 設定名は元のコードに合わせてください
                'U_FIXED_VAL': 2.7,
                'USE_PROBABILISTIC_STICKING': False,
                'USE_SURFACE_DIFFUSION_TRAPPING': False
            }
        },
        {
            "name": "2_dynamic",
            "updates": {
                'U_DIST_MODE': 'dynamic',
                'USE_PROBABILISTIC_STICKING': True,
                'USE_SURFACE_DIFFUSION_TRAPPING': False
            }
        },
        {
            "name": "3_gaussian_random_diffusion",
            "updates": {
                'U_DIST_MODE': 'gaussian_random',
                'USE_PROBABILISTIC_STICKING': True,
                'USE_SURFACE_DIFFUSION_TRAPPING': True  # surface_diffusionあり
            }
        }
    ]
    ...
    for scenario in scenarios:
        print(f"\n=========================================================")
        print(f"🌟 開始: シナリオ 【{scenario['name']}】")
        print(f"=========================================================")
        
        # 1. 辞書をコピーしてパラメータを上書き
        current_settings = copy.deepcopy(SIMULATION_SETTINGS)
        current_settings.update(scenario['updates'])
        current_settings['T_END_RUN'] = ONE_YEAR_SECONDS  # 1年目で確実に終了させる
        
        # 2. 出力ディレクトリをシナリオごとに分ける
        target_out_dir = f"./output_{scenario['name']}"
        os.makedirs(target_out_dir, exist_ok=True)
        
        # 3. メインシミュレーションを呼び出す
        # ※ main_snapshot_simulation が引数を受け取れるように変更が必要です
        try:
            main_snapshot_simulation(
                custom_settings=current_settings, 
                target_output_dir=target_out_dir,
                run_name=scenario['name']
            )
        except Exception as e:
            print(f"❌ エラー発生 ({scenario['name']}): {e}")
            # エラーが起きても次のシナリオに進むようにする
            continue 
