...
SIMULATION_RUN_DIRECTORY = r"./SimulationResult_202607/ParabolicHop_72x36_NoEq_DT100_0702_NonBD_UF1.85_Q2.0_A2.0e+07_LT190k"
...
def get_all_grid_files_sorted(target_dir):
    ...
    file_list = []
    for f in files:
        # 正規表現で _t(時間)_taa(TAA).npy を抽出 (最新のコード出力と完全互換)
...
class Grid3DViewer:
    def __init__(self, file_list, view_from='Z'):
        ...
        # ★ 総原子数プロットボタン
        ax_total_atoms = plt.axes([0.25, 0.025, 0.15, 0.04])
        self.btn_total_atoms = Button(ax_total_atoms, 'Plot Total Atoms')
        self.btn_total_atoms.on_clicked(self.generate_total_atoms_plot)
    ...
    def load_and_process(self, idx):
        ...
        try:
            ...
            #column_density = gaussian_filter(column_density, sigma=1.0)
            # 単位変換
    ...
    # ★ 総原子数プロット作成機能
    # ==========================================================================
    def generate_total_atoms_plot(self, event):
        """すべてのファイルを走査し、最後の軌道（最新のtime_h）のTAAごとの総原子数をプロットする"""
        print("\n--- Generating Total Atoms vs TAA Plot (Last Orbit Only) ---")
        print("データを読み込み中です。しばらくお待ちください...")
        
        # TAAごとに最新のファイル（＝最後の1周分/3年目）だけを辞書に抽出
        latest_files_dict = {}
        for info in self.file_list:
            taa = info['taa']
            time_h = info['time_h']
            # まだ辞書にない、または既存のデータより時間が進んでいれば上書き
            if taa not in latest_files_dict or time_h > latest_files_dict[taa]['time_h']:
                latest_files_dict[taa] = info
                
        # TAAの昇順に並び替え
        sorted_latest_files = sorted(latest_files_dict.values(), key=lambda x: x['taa'])
        
        taas = []
        total_atoms_list = []
        
        # 1セルの体積 [m^3] を計算
        cell_volume_m3 = self.cell_size_m ** 3
        center_idx = GRID_RESOLUTION // 2
        
        for info in sorted_latest_files:
            filepath = info['path']
            taa = info['taa']
            try:
                # 3Dグリッド [atoms/m^3] を読み込み
                #data_3d = np.load(filepath)
                # 総原子数 = 密度の合計 * 1セルの体積
                #total_atoms = np.sum(data_3d) * cell_volume_m3

                # 3Dグリッド [x, y, z] を読み込み
                data_3d = np.load(filepath)
                
                # x > 0 (昼側) の領域だけを切り出す (X軸は axis=0)
                # ※厳密に x>0 にする場合は center_idx + 1 にします
                data_dayside = data_3d[center_idx + 1:, :, :]
                
                # 昼側の総原子数 = 昼側密度の合計 * 1セルの体積
                total_atoms = np.sum(data_dayside) * cell_volume_m3
        ...
        # 新しいウィンドウでプロット
        fig_atoms, ax_atoms = plt.subplots(figsize=(8, 6))
        
        # 1周分に限定したので線で繋いでも問題ありませんが、ご指定通り線なしの設定(linestyle='')を維持します。
        ax_atoms.plot(taas, total_atoms_list, marker='o', linestyle='', color='b', alpha=0.8)
        
        #ax_atoms.set_title("Total Number of Atoms vs True Anomaly Angle (Last Orbit)", fontsize=14, fontweight='bold')
        ax_atoms.set_xlabel("True Anomaly Angle (TAA) [deg]", fontsize=12)
        ax_atoms.set_ylabel("Total Number of Atoms", fontsize=12)
        
        # --- 横軸と縦軸の範囲・メモリ設定 ---
        ax_atoms.set_xlim(0, 360)
        ax_atoms.set_xticks(np.arange(0, 361, 60))
        ax_atoms.set_ylim(bottom=0)
        
        ax_atoms.grid(True, linestyle='--', alpha=0.7)
        
        # y軸を対数スケールにしたい場合は以下のコメントアウトを外してください
        # ax_atoms.set_yscale('log')
    ...
    def generate_plate_view(self, event):
        ...
        for i, target_taa in enumerate(target_taas):
            ...
            # タイトルとラベル
            ax.set_title(f"TAA = {actual_taa}$^\circ$", fontsize=14, fontweight='bold')
            ax.set_aspect('equal')
