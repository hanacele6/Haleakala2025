...
def simulate_particle_for_one_step(args: Dict) -> Dict:
    ...
    while time_remaining > 1e-6:
        ...
        if is_hit:
            ...
            # 1. 吸着モデルの選択と判定
            # ==========================================================
            sticking_model = settings.get('STICKING_MODEL', 'empirical') # デフォルトは経験的モデル
            ...
            if sticking_model == 'baule':
                ...
                is_trapped, vel_rebound = evaluate_baule_collision(
                    ...
                    normal=norm_hit, 
                    U_0_eV=U_assigned, 
                    mass_target_amu=16.0#SiO2と仮定して、さらにOと仮定
                )
            else:
                # 【モデルB】空隙率を考慮した経験的確率判定
                prob = calculate_sticking_probability(temp_impact)
                if np.random.random() < prob:
                    is_trapped = True
                else:
                    is_trapped = False
                    # 経験的モデルで「弾かれた」場合、先行研究に倣いその場の温度で熱的バウンドとする
