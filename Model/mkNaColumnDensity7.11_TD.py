...
# ... (PHYSICAL_CONSTANTS, ヘルパー関数群は変更なし) ...
# (calculate_surface_temperature, calculate_sticking_probability, ...)
# (... sample_maxwellian_speed, sample_lambertian_direction_local, ...)
# (... transform_local_to_world, get_orbital_params, lonlat_to_xyz, ...)
# (... _calculate_acceleration, simulate_particle_for_one_step も変更なし) ...
...
def main_snapshot_simulation():
    ...
    # --- 変更: 固定粒子数(予算)設定 ---
    # 1タイムステップあたりに生成するスーパー粒子の「目標数」
    # これを増減させて N_particles が 1万程度になるよう調整する
    TARGET_SPS_TO_GENERATE_PER_STEP = 5000
    # --- 変更 ここまで ---
    ...
    # 出力ディレクトリの準備
    # --- 変更 (予算方式) ---
    run_name = f"Grid{GRID_RESOLUTION}_Budget{TARGET_SPS_TO_GENERATE_PER_STEP}_TD"
    # --- 変更 ここまで ---

    target_output_dir = os.path.join(OUTPUT_DIRECTORY, run_name)
    os.makedirs(target_output_dir, exist_ok=True)
    print(f"結果は '{target_output_dir}' に保存されます。")
    print(f"--- 物理モデル設定 (TD Only / 予算方式) ---")
    print(f"Binding Energy: {E_BINDING_EV} eV")
    print(f"Target SPs/Step: {TARGET_SPS_TO_GENERATE_PER_STEP}")
    ...
    # --- 軌道ファイルの TAA=0 (近日点) ... (ここのロジックは変更なし) ---
    ...
    # ★★★ 修正: tqdm表示用の変数を追加 ★★★
    current_step_weight = 0.0  # このステップの重みをtqdmに表示する用
    ...
    with tqdm(total=len(time_steps), desc="Time Evolution") as pbar:
        for t_sec in time_steps:
            ...
            pbar.set_description(
                f"[{run_phase}] TAA={TAA:.1f} | N_particles={len(active_particles)} | StepWeight={current_step_weight:.1e}"
            )

            # --- 4b. 表面から新しい粒子を生成 (★★★ 予算配分方式 ★★★) ---

            newly_launched_particles = []
            k_B = PHYSICAL_CONSTANTS['K_BOLTZMANN']

            # (A) 事前ループ: 惑星全体の「総放出原子数」をまず計算する
            total_atoms_this_step = 0.0
            # (このステップの全セルの放出量を保存する一時配列)
            cell_desorption_atoms = np.zeros((N_LON, N_LAT))
            ...
            for i_lon in range(N_LON):
                for i_lat in range(N_LAT):
                    ...
                    n_atoms = desorption_rate_per_m2_s * cell_areas_m2[i_lat] * TIME_STEP_SEC

                    if n_atoms > 0:
                        cell_desorption_atoms[i_lon, i_lat] = n_atoms
                        total_atoms_this_step += n_atoms

            # (B) 重みの決定: このステップで使う「共通の重み」を計算
            current_step_weight = 1.0  # ゼロ割を防ぐための初期値

            if total_atoms_this_step > 0:
                current_step_weight = total_atoms_this_step / TARGET_SPS_TO_GENERATE_PER_STEP
            else:
                # もし惑星全体で原子が1個も生成されなかったら (遠日点の夜側など)
                # ループをスキップしてよい
                active_particles.extend(newly_launched_particles)  # (空のリストを追加)
                # ... (4c, 4d, 4e に進む)
                pass  # (この下で active_particles.extend するので、ここでは何もしない)

            # (C) メインループ: 粒子を生成
            # (total_atoms_this_step > 0 の場合のみ実行)
            if total_atoms_this_step > 0:
                for i_lon in range(N_LON):
                    for i_lat in range(N_LAT):

                        n_atoms_to_desorb = cell_desorption_atoms[i_lon, i_lat]
                        if n_atoms_to_desorb <= 0:
                            continue

                        # ★★★ この計算方法の核心 ★★★
                        # num_sps = (セル原子数 / 共通の重み)
                        num_sps_to_launch_float = n_atoms_to_desorb / current_step_weight
                        # (例: 1e30 / 2e30 = 0.5個)
                        ...
                        # (粒子生成ロジックは変更なし)
                        lon_min, lon_max = lon_edges[i_lon], lon_edges[i_lon + 1]
                        sin_lat_min, sin_lat_max = np.sin(lat_edges[i_lat]), np.sin(lat_edges[i_lat + 1])

                        # (このセルの温度を再計算 - T_sを保存しておけば効率的だが、確実性を優先)
                        lon_center_rad = (lon_min + lon_max) / 2
                        lat_center_rad = (lat_edges[i_lat] + lat_edges[i_lat + 1]) / 2
                        T_s = calculate_surface_temperature(lon_center_rad, lat_center_rad, AU, 0.0)

                        for _ in range(num_to_launch_int):
                            random_lon_rad = np.random.uniform(lon_min, lon_max)
                            ...
                            newly_launched_particles.append({
                                'pos': initial_pos,
                                'vel': initial_vel,
                                'weight': current_step_weight  # ★★★ 共通の重みを設定 ★★★
                            })
