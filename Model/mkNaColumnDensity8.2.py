            ...
            for i_lon in range(N_LON_FIXED):
                for i_lat in range(N_LAT):
                    ...
                    for proc_name, p in procs.items():
                        ...
                        # ★★★ 万が一、計算誤差で枯渇量が利用可能量を超えた場合のための安全装置 ★★★
                        atoms_available_in_cell_for_proc = p['n_atoms']  # このプロセスが使える原子数
