...
2.  複数の生成過程:
    ...
    - 熱脱離 (TD): 表面密度と表面温度に比例。
    - 微小隕石蒸発 (MIV): 表面密度に依存しない補給源。MMVだったりMIVだったりする。
    - 太陽風イオンスパッタリング(SWS): 未実装。要勉強。 SWSだったりCPSだったりする。
...
def main_snapshot_simulation():
    ...
    SPIN_UP_YEARS = 3.0  # 動的平衡に達するため、長めのスピンアップ。Leblancは6年。suzukiは2年。
    TIME_STEP_SEC = 1000
    TOTAL_SIM_YEARS = 1.0
    TARGET_TAA_DEGREES = np.arange(0, 360, 1)  # TAAが0度から360°の間で1度ずつスナップショットを保存。
            ...
            for i_lon in range(N_LON_FIXED):
                for i_lat in range(N_LAT):
                    ...
                    for proc_name, p in procs.items():
                        ...
                        # MMVではない(PSD, TD, SWS)ので、表面密度を減らす
