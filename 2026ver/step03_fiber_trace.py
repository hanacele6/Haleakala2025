...
def create_final_trace_products(data_filepath, dark_filepath, output_dir, config, make_plots=True):
    ...
    if make_plots:
        print("\n--- 可視化プロットを生成します ---")
        png_dir = output_dir / "plots" / data_filepath.stem.replace("_nhp_py", "")
    ...
    print(f"視覚化マップを保存しました: {file_pp1.name}")

    file_fppoly = output_dir / f"{clean_stem}.fppoly.fits"
    ...
    print(f"高精度軌跡データを保存しました: {file_fppoly.name}")
