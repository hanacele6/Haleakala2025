...
def extract_spectra_final_IDL_replica(science_file_pattern, fppoly_path, dark_path, output_dir, config):
    ...
    for science_filepath_str in science_files:
        ...
        for ifib in range(num_fibers):
            ...
            # === デバッグ用コード START ===
            # 真ん中のファイバーの時だけ、中間データを表示して停止する
            if ifib == num_fibers // 2:
                print(f"\n\n--- ファイバー #{ifib} のデバッグ情報を表示します ---")

                print(f"\n[ステップ1] 初回補間後のデータストリップ")
                print(
                    f"  - provisional_strip (min, max): ({np.nanmin(provisional_strip):.2f}, {np.nanmax(provisional_strip):.2f})")

                print(f"\n[ステップ2] ガウスフィットへの入力")
                # avg_profile の値を表示（長くなりすぎないように整形）
                avg_profile_str = np.array2string(avg_profile, precision=2, separator=', ')
                print(f"  - avg_profile: {avg_profile_str}")

                print(f"\n[ステップ3] ガウスフィットの結果")
                try:
                    # 'par'が未定義の場合のエラーを避ける
                    par_str = np.array2string(par, precision=2, separator=', ')
                    print(f"  - フィットパラメータ (A, μ, σ, B): {par_str}")
                except NameError:
                    print("  - フィットが実行されなかったか、失敗しました。")
                print(f"  - 最終的に使用された中心位置: {center_in_strip:.4f}")

                print(f"\n[ステップ4] 2回目補間後のデータストリップ")
                print(
                    f"  - final_rectified_strip (min, max): ({np.nanmin(final_rectified_strip):.2f}, {np.nanmax(final_rectified_strip):.2f})")

                print(f"\n[ステップ5] 最終スペクトル計算の直前")
                # 最初の5点だけ表示
                print(f"  - 信号 (signal) の最初の5点: {np.array2string(signal[:5], precision=2)}")
                print(f"  - 背景 (background) の最初の5点: {np.array2string(background[:5], precision=2)}")

                print(f"\n[ステップ6] 計算された最終スペクトル")
                print(
                    f"  - final_spectrum (min, max): ({np.nanmin(final_spectrum):.2f}, {np.nanmax(final_spectrum):.2f})")
...
if __name__ == '__main__':
    ...
    config_old = {
        ...
        'output_flip_lr': True,  # ★★★ 左右反転を有効にする場合は True ★★★
    }
