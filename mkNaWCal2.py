...
for iy in range(iym):
    ...
    try:
        popt_coarse_d2, _ = curve_fit(gaussian_with_linear, x_coarse_d2, y_coarse_d2, p0=p0_coarse_d2, method='lm')
        c2 = (popt_coarse_d2[1] )+ d2s
    except RuntimeError:
        c2 = d2s + (d2e - d2s) / 2  # フィット失敗時は中央値
    ...
    try:
        popt_coarse_d1, _ = curve_fit(gaussian_with_linear, x_coarse_d1, y_coarse_d1, p0=p0_coarse_d1, method='lm')
        c1 = (popt_coarse_d1[1] ) + d1s
    except RuntimeError:
        c1 = d1s + (d1e - d1s) / 2  # フィット失敗時は中央値
    ...
    p0_fine_d2 = [-np.max(y_fine_d2), dw, 5.0, np.median(y_fine_d2), 0]
    try:
        popt_fine_d2, _ = curve_fit(gaussian_with_linear, x_fine_d2, y_fine_d2, p0=p0_fine_d2 , method='lm')
        c21[iy] = (popt_fine_d2[1] ) + fine_s2
    except RuntimeError:
        c21[iy] = c2

    # D1線（精密）
    # 注意：元のIDLコードでは、D1の精密フィットにもD2の中心(ic2)を使っていました。
    # ここではそのロジックを再現しますが、本来はic1を使うべきかもしれません。
    fine_s1, fine_e1 = ic1 - dw, ic1 + dw
    #fine_s1, fine_e1 = ic2 - dw, ic2 + dw # IDLのロジック
    x_fine_d1 = np.arange(fine_e1 - fine_s1 + 1)
    y_fine_d1 = comp_spec[iy, fine_s1:fine_e1 + 1]
    p0_fine_d1 = [-np.max(y_fine_d1), dw, 5.0, np.median(y_fine_d1), 0]
    try:
        popt_fine_d1, _ = curve_fit(gaussian_with_linear, x_fine_d1, y_fine_d1, p0=p0_fine_d1, method='lm')
...
for i in range(is_num, ie_num + 1):
    science_path = fileF1 / f"{i}_f_python.fit"
    ...
    try:
        ...
        for iy in range(iym):
            ...
            row_data = Mc[iy, :]
            # 補間関数を作成
            #interp_func = interp1d(x_coords, row_data, kind='linear', bounds_error=False, fill_value=0)
            interp_func = interp1d(x_coords, row_data, kind='linear', bounds_error=False, fill_value=(row_data[0], row_data[-1]))
        ...
        # 補正後のデータを保存
        output_path = fileF2 / f"{i}_Na_python.fit"
...
w2 = 589.1582 - 589.0
w1 = 589.7558 - 589.0
with open(fileF2 / 'pix2spec_python.dat', 'w') as f:
