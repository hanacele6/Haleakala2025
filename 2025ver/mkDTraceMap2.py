...
def create_final_trace_products(data_filepath, dark_filepath, output_dir, config, make_plots=True):
    ...
    print("\n--- ファイバーアレイの全体的な位置ズレを自己補正します ---")
    first_fiber = iFibAct[0]
    j0 = xpixF[0]
    m2_first = yfibs[first_fiber]

    # 基準となるフィット窓を定義
    ypix1_first = (np.arange(ypixFibWid) - ypixFibWid / 2 + 0.5 + m2_first).astype(int)

    if not (ypix1_first.min() < 0 or ypix1_first.max() >= num_spatial_pts):
        ydat1_first = data[ypix1_first, j0]
        try:
            # --- ▼▼▼ 初期値と探索範囲を正しく設定 ▼▼▼ ---
            window_mean_first = np.mean(ypix1_first)
            Aini_first = [np.max(ydat1_first), window_mean_first, ypixFibWid / 5.0]
            param_bounds_first = ([1, window_mean_first - ypixFibWid / 2, 0.1],
                                  [1e5, window_mean_first + ypixFibWid / 2, 3])

            # p0とboundsを指定してフィットを実行
            par_first, _ = curve_fit(gaussian, ypix1_first, ydat1_first,
                                     p0=Aini_first, bounds=param_bounds_first, max_nfev=2000)

            # 理論値と実際のフィット値の差分（dlt）を計算
            dlt = par_first[1] - yfibs[first_fiber]
            print(f"検出された全体オフセット: {dlt:.2f} ピクセル")

            # 全てのファイバーの初期位置をこの差分で補正する
            yfibs += dlt
            print("全ファイバーの初期位置を補正しました。")

        except RuntimeError:
            print("警告: 自己補正フィットに失敗。configの初期値をそのまま使用します。")
    ...
    for i in iFibAct:
        ...
        #for j in reversed(xpixF):
            ...
            param_bounds = ([1, window_mean - ypixFibWid/2, 0.1],  # 探索範囲の下限
                            [1e5, window_mean + ypixFibWid/2, 3])  # 探索範囲の上限
            try:
        ...
        # この後のfppoly_dataへの代入も、上書き後のAARRから行う
        ...
        if make_plots:
            ...
            # 元コードは少し広い範囲(ypix0)もプロットしているので、それを再現
            ...
            # 6. タイトルを元コードの形式に合わせる
            fit_center_val = AARR[j0, i, 1] if np.isfinite(AARR[j0, i, 1]) else np.nan
            ax1.set_title(f'iFib={i} j={j0} center={fit_center_val:.2f}', fontsize=6)
    ...
    try:
        ...
        if target_rows.empty:
            ...
            if not input_file.exists():
                ...
                create_final_trace_products(input_file, dark_file, output_dir, config, make_plots=True)#trueだとpngできます
