...
for ifile, original_filepath_str in enumerate(file_list_original):
    ...
    for ifib in range(ifibm):
        ...
        for ix in range(NX):
            ...
            interp_func_col = interp1d(
                y_pixel_indices, b[:, ix],
    ...
    hdu = fits.PrimaryHDU(fiblall2.astype(np.float32))
    # 元画像のヘッダー情報を引き継ぐ場合はここで追加
    #hdu.header.extend(original_header)
